...
function defaultData(){
  return {
    ...
    settings:{
      ...
      defaultBI:10,
      defaultIVA:21,
      defaultObservations:'Revisar PU abans d\'enviar.\nPendent de valoració actualitzada.',
      contractista:{
...
function budgetFormCard(formBudget,isNew=false){
  return `<div class="card"><h2>${isNew?'Nou pressupost':'Editar pressupost'}</h2>
      <form id="budgetForm" class="form-grid">
        ...
        <label class="full">Observacions / notes<textarea name="notes">${esc(formBudget.notes||'')}</textarea></label>
  ...
  setContent(`
    <div class="card"><h2>${editId?'Editar factura':'Nova factura'}</h2>
      <form id="invoiceForm" class="form-grid">
        ...
        <label class="wide">Client<select name="clientId"><option value="">Client pendent de validar</option>${options(data.clients,i.clientId,c=>c.name||c.id)}</select></label>
        ...
        <div class="actions full"><button class="primary">Guardar factura</button>${editId?`<button class="ghost" type="button" data-preview-invoice-form="${esc(i.id)}">Vista preliminar A4 / PDF</button><button class="ghost" type="button" data-render-invoices>Cancel·lar</button>`:''}</div>
      </form>
    </div>
...
function updateBudgetLine(e){ const b=byId(data.budgets,state.selectedBudgetId); if(!b) return; const l=(b.lines||[]).find(x=>x.id===e.target.dataset.lineId); if(!l) return; const field=e.target.dataset.lineField; l[field]=['qty','unitPrice'].includes(field)?num(e.target.value):e.target.value; if(field==='qty' || field==='unitPrice') l.total=num(l.qty)*num(l.unitPrice); b.manualLinesEdited=true; saveData(); if(isModalOpen()) openBudgetModal(b.id); else renderBudgets(); }
function deleteBudgetLine(id){ const b=byId(data.budgets,state.selectedBudgetId); if(!b) return; b.manualLinesEdited=true; b.lines=(b.lines||[]).filter(x=>x.id!==id); saveData(); if(isModalOpen()) openBudgetModal(b.id); else renderBudgets(); }
function deleteSelectedBudgetLines(){ const ids=selectedValues('.select-budget-line'); if(!ids.length) return alert('No has seleccionat cap línia.'); const b=byId(data.budgets,state.selectedBudgetId); if(!b) return; if(!confirm(`Eliminar ${ids.length} línia/es seleccionada/es del pressupost?`)) return; b.manualLinesEdited=true; b.lines=(b.lines||[]).filter(l=>!ids.includes(l.id)); saveData(); if(isModalOpen()) openBudgetModal(b.id); else renderBudgets(b.id); }

function addManualLine(){ const b=byId(data.budgets,state.selectedBudgetId); if(!b) return; b.manualLinesEdited=true; b.lines=b.lines||[]; b.lines.push({id:uid('LIN'),code:'',chapter:'',unit:'',concept:'Nova partida',longDesc:'',qty:1,unitPrice:0,total:0,status:'Manual pendent revisar',origin:'Manual'}); saveData(); if(isModalOpen()) openBudgetModal(b.id); else renderBudgets(); }
...
saveInvoice=function(event){
  ...
  const invoice={...old,id:fields.id,number:fields.number,date:fields.date,clientId:fields.clientId||old.clientId||'',jobId:fields.jobId,budgetId:fields.budgetId,type:fields.type,concept:fields.concept,base,iva,total:Number((base*(1+iva/100)).toFixed(2)),paid:fields.paid==='true',notes:fields.notes};
...
function teimor0913v2Accept(value,kind,sameRow){
  ...
  if(kind==='concept'){
    /* El concepte pot descriure una feina (impermeabilització, pintura,
       coberta...), per tant no l'hem de rebutjar només perquè s'assembli a
       una línia de partida. Aquí només acceptem el valor de l'etiqueta
       exacta «Concepte/Concepto». */
    return t.length>1 && t.length<=240 ? t : '';
  }
  if(kind==='address'){
...
bindViewEvents=function(){
  ...
  ['v09133InvoiceSearch','v09133InvoiceYear','v09133InvoiceClient','v09133InvoiceStatus','v09134InvoiceRelation'].forEach(id=>{ const element=document.getElementById(id); if(element) element.oninput=filterInvoicesV09133; if(element) element.onchange=filterInvoicesV09133; });
...
/* =========================================================
   TEIMOR V09.13.4 · SEPARACIÓ DE DOCUMENTS, COMPTADORS I PDF
   ========================================================= */

data.meta = data.meta || {};
data.meta.version = '9.13.4-separacio-comptadors-pdf';

function teimor09134BudgetLooksInvoice(budget){
  const item=budget||{};
  const kind=strip(item.kind||item.documentType||item.type||'');
  if(/^(factura|factures|invoice|invoices)$/.test(kind)) return true;
  if(/^(pressupost|pressupostos|budget|budgets)$/.test(kind)) return false;
  if(item.attachmentId || item.sourceFileName || item.clientSnapshot || item.matchStatus) return true;
  const origin=strip([item.source,item.sourceFile,item.number,item.title,item.keyword,item.notes].filter(Boolean).join(' '));
  return /\b(?:factura|factures|invoice|invoices|fra)\b/.test(origin);
}

function teimor09134InvoiceKey(item){
  const number=teimor0913NormId(item?.number||'');
  const date=String(item?.date||'');
  const source=teimor0913NormId(item?.sourceFile||item?.source||'');
  return number ? number+'|'+date : source;
}

function teimor09134InvoiceFromBudget(budget){
  const client=byId(data.clients,budget.clientId)||{};
  const base=num(budget.base)>0 ? num(budget.base) : (num(budget.importedBase)>0 ? num(budget.importedBase) : budgetBase(budget));
  const iva=num(budget.iva ?? data.settings.defaultIVA);
  const total=num(budget.total)>0 ? num(budget.total) : Number((base*(1+iva/100)).toFixed(2));
  const existing=(data.invoices||[]).find(item=>teimor09134InvoiceKey(item)===teimor09134InvoiceKey(budget));
  const id=existing?.id || (budget.id && !(data.invoices||[]).some(item=>item.id===budget.id) ? budget.id : uid('FAC'));
  return {
    ...(existing||{}),
    id,
    kind:'factura',
    ...
    type:'Client',
    number:budget.number||budget.oldNumber||budget.id||id,
    date:budget.date||'',
    ...
    clientSnapshot:budget.clientSnapshot||{
      ...
      city:client.city||'',
      workAddress:budget.workAddress||byId(data.jobs,budget.jobId)?.address||client.workAddress||'',
      workCity:budget.workCity||byId(data.jobs,budget.jobId)?.city||client.workCity||client.city||'',
      workPostalCode:budget.workPostalCode||byId(data.jobs,budget.jobId)?.postalCode||client.workPostalCode||client.postalCode||''
    },
    jobId:budget.jobId||'',
    budgetId:'',
    concept:budget.concept||budget.keyword||budget.title||'Factura importada',
    base:Number(base.toFixed(2)),
    iva:Number(iva.toFixed(2)),
    ivaAmount:Number((total-base).toFixed(2)),
    total:Number(total.toFixed(2)),
    paid:!!budget.paid,
    status:budget.status && strip(budget.status)!=='historic importat' ? budget.status : 'Importada · pendent de revisar',
    matchStatus:budget.matchStatus||'pendent',
    matchConfidence:num(budget.matchConfidence),
    sourceFile:budget.sourceFile||budget.source||'',
    sourceFileName:budget.sourceFileName||'',
    sourceFiles:budget.sourceFiles||[budget.sourceFile||budget.source].filter(Boolean),
    notes:[budget.notes,'Registre separat automàticament de Pressupostos perquè corresponia a una factura.'].filter(Boolean).join('\n'),
    legacyBudgetId:budget.id,
    fileStatus:budget.fileStatus||'Fitxer original pendent de guardar'
  };
}

function teimor09134JobFinancialOnly(job){
  const blob=strip([job?.title,job?.keyword,job?.source,job?.notes].filter(Boolean).join(' '));
  return /\b(?:factura|factures|invoice|invoices|certificacio|certificacions)\b/.test(blob);
}

function teimor09134JobKey(job){
  const client=teimor0913NormId(job?.clientId||'');
  const address=teimor09132AddressText(job?.workAddress||job?.address||'');
  const keyword=teimor0913Norm(job?.keyword||job?.title||'');
  const generic=!keyword || /^(?:concepte pendent de revisar|obra sense nom|feina sense nom|pendent de revisar)$/.test(keyword);
  if(client && address) return client+'|'+address;
  if(client && !generic && keyword) return client+'|'+keyword+'|'+String(job?.year||'');
  return '';
}

function teimor09134JobReferences(id){
  return (data.budgets||[]).filter(item=>item.jobId===id).length+
    (data.invoices||[]).filter(item=>item.jobId===id).length+
    (data.certifications||[]).filter(item=>item.jobId===id).length+
    (data.attachments||[]).filter(item=>item.jobId===id).length+
    (data.agenda||[]).filter(item=>item.jobId===id).length;
}

function teimor09134MergeJob(target,source){
  const sourceKeyword=teimor0913Norm(source?.keyword||source?.title||'');
  const targetKeyword=teimor0913Norm(target?.keyword||target?.title||'');
  if(sourceKeyword && targetKeyword && sourceKeyword!==targetKeyword){
    target.alternativeKeywords=[...(target.alternativeKeywords||[]),source.keyword||source.title].filter(Boolean).filter((value,index,array)=>array.findIndex(item=>teimor0913Norm(item)===teimor0913Norm(value))===index);
  }
  ['year','clientId','title','keyword','address','workAddress','city','workCity','postalCode','workPostalCode','status','mainBudgetId','notes'].forEach(field=>{
    if((target[field]===undefined || target[field]===null || target[field]==='') && source[field]!==undefined && source[field]!==null && source[field]!=='') target[field]=source[field];
  });
  return target;
}

function teimor09134RemapJobIds(jobMap){
  if(!jobMap.size) return false;
  let changed=false;
  ['budgets','invoices','certifications','attachments','agenda'].forEach(collection=>{
    (data[collection]||[]).forEach(item=>{
      if(item.jobId && jobMap.has(item.jobId)){ item.jobId=jobMap.get(item.jobId); changed=true; }
    });
  });
  if(state.selectedJobId && jobMap.has(state.selectedJobId)){ state.selectedJobId=jobMap.get(state.selectedJobId); changed=true; }
  return changed;
}

function teimor09134RepairData(){
  data.clients=Array.isArray(data.clients)?data.clients:[];
  data.jobs=Array.isArray(data.jobs)?data.jobs:[];
  data.budgets=Array.isArray(data.budgets)?data.budgets:[];
  data.invoices=Array.isArray(data.invoices)?data.invoices:[];
  ...
  data.attachments=Array.isArray(data.attachments)?data.attachments:[];
  data.agenda=Array.isArray(data.agenda)?data.agenda:[];
  let changed=false, movedInvoices=0, mergedJobs=0, removedGhostJobs=0;

  const budgetRows=[];
  for(const budget of data.budgets){
    if(!teimor09134BudgetLooksInvoice(budget)){
      if(budget.documentType!=='pressupost'){ budget.documentType='pressupost'; changed=true; }
      budgetRows.push(budget);
      continue;
    }
    const converted=teimor09134InvoiceFromBudget(budget);
    const existing=data.invoices.find(item=>teimor09134InvoiceKey(item)===teimor09134InvoiceKey(converted));
    if(existing){
      Object.assign(existing,converted);
    }else{
      data.invoices.push(converted);
    }
    movedInvoices++;
    changed=true;
  }
  if(budgetRows.length!==data.budgets.length) data.budgets=budgetRows;

  data.invoices.forEach(invoice=>{
    if(invoice.kind!=='factura'){ invoice.kind='factura'; changed=true; }
    if(invoice.documentType!=='factura'){ invoice.documentType='factura'; changed=true; }
  });
  data.certifications.forEach(cert=>{
    if(cert.kind!=='certificacio'){ cert.kind='certificacio'; changed=true; }
    if(cert.documentType!=='certificacio'){ cert.documentType='certificacio'; changed=true; }
  });

  const ordered=[...data.jobs].sort((a,b)=>teimor09134JobReferences(b)-teimor09134JobReferences(a) || (b.mainBudgetId?1:0)-(a.mainBudgetId?1:0));
  const byKey=new Map(), jobMap=new Map(), kept=[];
  ordered.forEach(job=>{
    if(!job.id) job.id=uid('F');
    const key=teimor09134JobKey(job);
    if(key && byKey.has(key)){
      const canonical=byKey.get(key);
      teimor09134MergeJob(canonical,job);
      jobMap.set(job.id,canonical.id);
      mergedJobs++;
      changed=true;
    }else{
  ...
  if(teimor09134RemapJobIds(jobMap)) changed=true;
  data.jobs=kept;

  const budgetJobIds=new Set(data.budgets.map(item=>item.jobId).filter(Boolean));
  const docJobIds=new Set([...data.invoices,...data.certifications].map(item=>item.jobId).filter(Boolean));
  const ghostIds=new Set(data.jobs.filter(job=>teimor09134JobFinancialOnly(job) && !budgetJobIds.has(job.id) && docJobIds.has(job.id)).map(job=>job.id));
  if(ghostIds.size){
    data.jobs=data.jobs.filter(job=>!ghostIds.has(job.id));
    [...data.invoices,...data.certifications].forEach(doc=>{
      if(ghostIds.has(doc.jobId)){ doc.jobId=''; changed=true; }
    });
    removedGhostJobs=ghostIds.size;
    changed=true;
  }

  [...data.invoices,...data.certifications].forEach(doc=>{
    if(doc.budgetId && !byId(data.budgets,doc.budgetId)){ doc.budgetId=''; changed=true; }
    if(doc.jobId && !byId(data.jobs,doc.jobId)){ doc.jobId=''; changed=true; }
    if(doc.matchStatus!=='manual' && (!doc.jobId || !doc.budgetId) && typeof teimor0913ApplyMatch==='function'){
      const before=JSON.stringify({clientId:doc.clientId||'',jobId:doc.jobId||'',budgetId:doc.budgetId||'',matchStatus:doc.matchStatus||'',matchReason:doc.matchReason||''});
      teimor0913ApplyMatch(doc);
      const after=JSON.stringify({clientId:doc.clientId||'',jobId:doc.jobId||'',budgetId:doc.budgetId||'',matchStatus:doc.matchStatus||'',matchReason:doc.matchReason||''});
      if(before!==after) changed=true;
    }
  });

  const previous=data.meta.v09134RepairSummary;
  const summary={movedInvoices,mergedJobs,removedGhostJobs,remainingBudgets:data.budgets.length,remainingInvoices:data.invoices.length,remainingJobs:data.jobs.length};
  if(JSON.stringify(previous)!==JSON.stringify(summary)){ data.meta.v09134RepairSummary=summary; changed=true; }
  return {changed,summary};
}

function teimor09134RepairCounts(){
  return {
    jobs:data.jobs.length,
    budgets:data.budgets.length,
    invoices:data.invoices.length,
    certifications:data.certifications.length,
    budgetBase:data.budgets.reduce((sum,item)=>sum+budgetBase(item),0),
    invoiceTotal:data.invoices.reduce((sum,item)=>sum+invoiceTotal(item),0)
  };
}

const __teimorBaseBudgetRowsV09134=budgetRowsFiltered;
budgetRowsFiltered=function(){
  return __teimorBaseBudgetRowsV09134().filter(item=>!teimor09134BudgetLooksInvoice(item));
};

function teimor09134InvoiceIsRelated(invoice){
  return !!(invoice?.jobId||invoice?.budgetId||invoice?.matchStatus==='automatica'||invoice?.matchStatus==='suggerida');
}

const __teimorBaseInvoiceRowsV09134=teimor09133InvoiceRows;
teimor09133InvoiceRows=function(){
  const rows=__teimorBaseInvoiceRowsV09134();
  const filter=document.getElementById('v09134InvoiceRelation')?.value ?? state.invoiceRelationFilter ?? '';
  if(filter==='relacionada') return rows.filter(teimor09134InvoiceIsRelated);
  if(filter==='pendent') return rows.filter(invoice=>!teimor09134InvoiceIsRelated(invoice));
  if(filter==='automatica') return rows.filter(invoice=>invoice.matchStatus==='automatica');
  return rows;
};

const __teimorBaseInvoiceListCardHtmlV09134=teimor09133InvoiceListCardHtml;
teimor09133InvoiceListCardHtml=function(){
  const counts=teimor09134RepairCounts();
  const relation=state.invoiceRelationFilter||'';
  const relationLabel='<label>Relació<select id="v09134InvoiceRelation"><option value="">Totes</option><option value="relacionada" '+(relation==='relacionada'?'selected':'')+'>Relacionades</option><option value="pendent" '+(relation==='pendent'?'selected':'')+'>Pendents</option><option value="automatica" '+(relation==='automatica'?'selected':'')+'>Automàtiques</option></select></label>';
  let html=__teimorBaseInvoiceListCardHtmlV09134();
  html=html.replace('<div class="filter-grid compact-filters">','<div class="grid four compact-kpis"><div class="kpi"><span>Total factures</span><strong>'+counts.invoices+'</strong></div><div class="kpi"><span>Relacionades</span><strong>'+data.invoices.filter(teimor09134InvoiceIsRelated).length+'</strong></div><div class="kpi"><span>Pendents</span><strong>'+data.invoices.filter(invoice=>!teimor09134InvoiceIsRelated(invoice)).length+'</strong></div><div class="kpi"><span>Total facturat</span><strong>'+money(counts.invoiceTotal)+'</strong></div></div><div class="filter-grid compact-filters">');
  html=html.replace('</label></div>\n    <div id="v09133InvoiceFilterInfo"',relationLabel+'</div>\n    <div id="v09133InvoiceFilterInfo"');
  html=html.replace('<button class="ghost" data-go="importer">Importar certificacions</button>','<button class="primary" data-v09134-auto-relate>Relacionar pendents</button><button class="ghost" data-go="importer">Importar certificacions</button>');
  return html;
};

const __teimorBaseInvoiceTableHtmlV09134=teimor09133InvoiceTableHtml;
teimor09133InvoiceTableHtml=function(){
  const html=__teimorBaseInvoiceTableHtmlV09134();
  return html.replace(/(<button class="ghost small" data-edit-invoice="([^"]+)">Editar<\/button>)/g,'<button class="ghost small" data-v09134-invoice-pdf="$2">Veure PDF</button> $1');
};

function teimor09134OpenInvoiceEditor(id){
  if(!byId(data.invoices,id)) return alert('No s’ha trobat aquesta factura.');
  state.view='invoices';
  renderInvoices(id);
}
...
function openInvoicePreview(id){
  const invoice=byId(data.invoices,id);
  if(!invoice) return alert('No s’ha trobat aquesta factura.');
  const client=byId(data.clients,invoice.clientId)||{};
  const snapshot=invoice.clientSnapshot||{};
  const job=byId(data.jobs,invoice.jobId)||{};
  const budget=byId(data.budgets,invoice.budgetId)||{};
  const s=data.settings.contractista||{};
  const name=client.name||snapshot.name||'Client pendent de revisar';
  const nif=client.nif||snapshot.nif||'';
  const fiscal=[client.fiscalAddress||snapshot.fiscalAddress,client.postalCode||snapshot.postalCode,client.city||snapshot.city].filter(Boolean).join(' · ');
  const workAddress=snapshot.workAddress||job.address||job.workAddress||budget.workAddress||'';
  const workCity=snapshot.workCity||job.city||job.workCity||budget.workCity||'';
  const concept=invoice.concept||'Factura importada';
  const html='<div class="preview-toolbar actions"><button class="primary" onclick="window.print()">Imprimir / guardar PDF</button><button class="ghost" onclick="window.close()">Tancar</button></div>'+
    '<div class="a4-sheet"><div class="preview-colorbar"></div>'+
    '<div class="preview-header"><div><h1>'+esc(s.name||'TEIMOR')+'</h1><p>'+esc(s.nif||'')+'<br>'+esc(s.address||'')+'<br>'+esc(s.city||'')+'<br>'+esc(s.phone||'')+'</p></div>'+
    '<div class="client-box"><span>Client</span><strong>'+esc(name)+'</strong><br>'+esc(nif)+'<br>'+esc(fiscal)+'</div></div>'+
    '<div class="preview-meta"><div><strong>Data:</strong> '+dateDisplay(invoice.date)+'</div><div><strong>Factura núm.:</strong> '+esc(invoice.number||invoice.id)+'</div><div><strong>Estat:</strong> '+esc(invoice.paid?'Pagada':(invoice.status||'Pendent'))+'</div></div>'+
    '<h2>'+esc(concept)+'</h2>'+
    (workAddress||workCity?'<p><strong>Obra:</strong> '+esc([workAddress,workCity].filter(Boolean).join(' · '))+'</p>':'')+
    '<table class="preview-table"><thead><tr><th>Concepte / descripció</th><th>Base s/IVA</th><th>IVA</th><th>Total</th></tr></thead><tbody><tr><td>'+esc(concept)+(invoice.notes?'<div class="preview-desc">'+esc(invoice.notes)+'</div>':'')+'</td><td class="num">'+money(invoiceBase(invoice))+'</td><td class="num">'+num(invoice.iva).toFixed(2)+'%</td><td class="num">'+money(invoiceTotal(invoice))+'</td></tr></tbody></table>'+
    '<div class="preview-totals"><div>Base s/IVA: <strong>'+money(invoiceBase(invoice))+'</strong></div><div>IVA '+num(invoice.iva).toFixed(2)+'%: <strong>'+money(invoiceTotal(invoice)-invoiceBase(invoice))+'</strong></div><div>Total: <strong>'+money(invoiceTotal(invoice))+'</strong></div></div>'+
    '<div class="preview-notes">'+esc([invoice.budgetId?'Pressupost relacionat: '+budgetName(invoice.budgetId):'',invoice.jobId?'Obra relacionada: '+jobName(invoice.jobId):''].filter(Boolean).join('\n'))+'</div></div>';
  if(typeof window==='undefined' || typeof window.open!=='function') return alert('El navegador no permet obrir la vista PDF.');
  const w=window.open('', '_blank');
  if(!w) return alert('El navegador ha bloquejat la finestra de previsualització. Permet pop-ups per aquesta app.');
  w.document.write('<!doctype html><html lang="ca"><head><meta charset="utf-8"><title>Factura '+esc(invoice.number||invoice.id)+'</title><style>'+previewCss()+'</style></head><body>'+html+'</body></html>');
  w.document.close();
}

const __teimorBaseFilterInvoicesV09134=filterInvoicesV09133;
filterInvoicesV09133=function(event){
  if(event?.target?.id==='v09134InvoiceRelation') state.invoiceRelationFilter=event.target.value||'';
  __teimorBaseFilterInvoicesV09134(event);
};

const __teimorBaseBindInvoiceActionsV09134=teimor09133BindInvoiceActions;
teimor09133BindInvoiceActions=function(){
  __teimorBaseBindInvoiceActionsV09134();
  document.querySelectorAll('#v09133InvoiceTable [data-edit-invoice]').forEach(button=>button.onclick=()=>teimor09134OpenInvoiceEditor(button.dataset.editInvoice));
  document.querySelectorAll('#v09133InvoiceTable [data-v09134-invoice-pdf]').forEach(button=>button.onclick=()=>openInvoicePreview(button.dataset.v09134InvoicePdf));
  document.querySelectorAll('[data-preview-invoice-form]').forEach(button=>button.onclick=()=>openInvoicePreview(button.dataset.previewInvoiceForm));
  document.querySelectorAll('[data-v09134-auto-relate]').forEach(button=>button.onclick=teimor09133RelateAllPending);
};

const __teimorBaseFilterObresV09134=teimor09133FilterObres;
teimor09133FilterObres=function(event){
  __teimorBaseFilterObresV09134(event);
  teimor09134UpdateObresSummary();
};

function teimor09134UpdateObresSummary(){
  const info=document.getElementById('v09134ObresFilterInfo');
  if(!info) return;
  const rows=[...document.querySelectorAll('[data-v09133-obres-row]')];
  const visible=rows.filter(row=>row.style.display!=='none').length;
  const counts=teimor09134RepairCounts();
  info.textContent='Mostrant '+visible+' de '+counts.jobs+' obres · '+counts.budgets+' pressupostos · '+counts.invoices+' factures.';
}

const __teimorBaseAddObresControlsV09134=teimor09133AddObresControls;
teimor09133AddObresControls=function(){
  __teimorBaseAddObresControlsV09134();
  const controls=document.getElementById('v09133ObresControls');
  if(!controls) return;
  if(!document.getElementById('v09134ObresFilterInfo')) controls.insertAdjacentHTML('beforeend','<div id="v09134ObresFilterInfo" class="small-text" style="margin-top:10px"></div>');
  teimor09134UpdateObresSummary();
};

const __teimorBaseRenderV09134=render;
render=function(){
  const repair=teimor09134RepairData();
  const result=__teimorBaseRenderV09134();
  if(repair.changed) saveData();
  return result;
};
...
/* =========================================================
   ...
   - Índexs en memòria per evitar cercar tots els documents per cada obra.
   - Filtres per any, client, obra, estat i estat de vinculació.
   - Paginació per no construir centenars d’elements DOM alhora.
   - Fitxa d’obra amb pestanyes de resum, pressupostos, factures,
     documentació, albarans, notes, temps/materials i rendiment.
   ========================================================= */
...
(function(){
  ...
  function mergeBudget(existing,incoming,jobId,clientId){
    ...
    const currentNotes=String(existing.notes||'').split(/\r?\n+/).map(value=>value.trim()).filter(Boolean);
    const notesAreGenerated=!currentNotes.length || currentNotes.every(value=>/^(?:pressupost importat|importada autom[aà]ticament|treballs separats per marcador|revisar pu abans d['’]enviar|pendent de valoraci[oó] actualitzada)/i.test(strip(value)));
    if(notesAreGenerated && String(inc.notes||'').trim()) existing.notes=inc.notes;
    if(Array.isArray(inc.observations)&&inc.observations.length && (!Array.isArray(existing.observations)||!existing.observations.length)) existing.observations=inc.observations;
    const importedLines=Array.isArray(existing.lines)&&existing.lines.length&&existing.lines.every(line=>line && !line.manual && !line.libraryId && !/^manual$|^llibreria$/i.test(String(line.origin||'')));
    const cleanedIncoming=/^9\.(?:15|16)\./.test(String(inc.importParserVersion||'')) && (num(inc.parseStats?.observationLines)>0 || num(inc.parseStats?.measurementLinesRemoved)>0 || num(inc.parseStats?.genericWorkItems)>0 || num(inc.parseStats?.legacyStructuredWorkItems)>0);
    if(cleanedIncoming && importedLines && Array.isArray(inc.lines)&&inc.lines.length && !existing.manualLinesEdited){
      existing.lines=inc.lines;
      existing.importParserVersion=inc.importParserVersion;
      existing.parseStats=inc.parseStats||existing.parseStats;
      existing.observations=Array.isArray(inc.observations)?inc.observations:existing.observations;
      existing.workItemsMode=inc.workItemsMode||existing.workItemsMode;
      existing.workItemsCount=inc.workItemsCount||existing.workItemsCount;
    }
...
function teimor09136MapPush(map,key,item){
  if(!key || !item) return;
...
function teimor09136IndexData(){
  const budgetsByJob=new Map(), invoicesByJob=new Map(), certificationsByJob=new Map();
  const attachmentsByJob=new Map(), agendaByJob=new Map(), hoursByJob=new Map(), materialsByJob=new Map();
  const budgetsById=new Map((data.budgets||[]).map(item=>[item.id,item]));
  (data.budgets||[]).forEach(item=>teimor09136MapPush(budgetsByJob,item.jobId,item));
  const addDocument=(map,doc)=>{
    const budget=doc?.budgetId?budgetsById.get(doc.budgetId):null;
    teimor09136MapPush(map,doc?.jobId,doc);
    teimor09136MapPush(map,budget?.jobId,doc);
  };
  (data.invoices||[]).forEach(doc=>addDocument(invoicesByJob,doc));
  (data.certifications||[]).forEach(doc=>addDocument(certificationsByJob,doc));
  (data.attachments||[]).forEach(doc=>addDocument(attachmentsByJob,doc));
  (data.agenda||[]).forEach(doc=>teimor09136MapPush(agendaByJob,doc.jobId,doc));
  (Array.isArray(data.hores)?data.hores:[]).forEach(doc=>teimor09136MapPush(hoursByJob,doc.jobId,doc));
  (Array.isArray(data.materials)?data.materials:[]).forEach(doc=>teimor09136MapPush(materialsByJob,doc.jobId,doc));
  (data.budgets||[]).forEach(budget=>{
    (budget.lines||[]).filter(line=>/material/i.test(String(line.type||line.category||''))).forEach(line=>{
      const material={...line,budgetId:budget.id,jobId:budget.jobId,sourceBudget:budget.number||budget.id};
      teimor09136MapPush(materialsByJob,budget.jobId,material);
    });
  });
  return {budgetsByJob,invoicesByJob,certificationsByJob,attachmentsByJob,agendaByJob,hoursByJob,materialsByJob,budgetsById};
}

function teimor09136JobYear(job,budgets){
  if(job?.year) return Number(job.year);
  const first=(budgets||[])[0];
  return first?Number(budgetYear(first)||0):0;
}

function teimor09136BuildModels(){
  const index=teimor09136IndexData();
  const models=(data.jobs||[]).map(job=>{
    const budgets=index.budgetsByJob.get(job.id)||[];
    const invoices=index.invoicesByJob.get(job.id)||[];
    const certifications=index.certificationsByJob.get(job.id)||[];
    const attachments=index.attachmentsByJob.get(job.id)||[];
    const agenda=index.agendaByJob.get(job.id)||[];
    const hours=index.hoursByJob.get(job.id)||[];
    const materials=index.materialsByJob.get(job.id)||[];
    const client=byId(data.clients,job.clientId)||{};
    const keyword=teimor0913v2JobKeyword(job)||job.keyword||job.title||'';
    const address=teimor0913v2JobAddress(job)||job.address||job.workAddress||'';
    const city=teimor0913v2JobCity(job)||job.city||job.workCity||'';
    const year=teimor09136JobYear(job,budgets);
    const blob=strip([
      job.id,year,keyword,address,city,job.status,client.name,client.nif,client.city,
      budgets.map(item=>[item.number,item.title,item.concept].join(' ')).join(' '),
      invoices.map(item=>[item.number,item.concept,item.date].join(' ')).join(' ')
    ].join(' '));
    return {job,client,budgets,invoices,certifications,attachments,agenda,hours,materials,keyword,address,city,year,blob,
      budgetTotal:budgets.reduce((sum,item)=>sum+budgetBase(item),0),
      invoiceBase:invoices.reduce((sum,item)=>sum+invoiceBase(item),0),
      invoiceTotal:invoices.reduce((sum,item)=>sum+invoiceTotal(item),0),
      certificationTotal:certifications.reduce((sum,item)=>sum+teimor0913CertTotal(item),0)};
  });
  models.sort((a,b)=>b.year-a.year || a.keyword.localeCompare(b.keyword,'ca') || a.address.localeCompare(b.address,'ca'));
  return {models,index};
}

function teimor09136ObresFilters(){
  return {
    q:strip(document.getElementById('v09136ObresSearch')?.value ?? state.obresSearch ?? ''),
    year:document.getElementById('v09136ObresYear')?.value ?? state.obresYearFilter ?? '',
    client:document.getElementById('v09136ObresClient')?.value ?? state.obresClientFilter ?? '',
    status:strip(document.getElementById('v09136ObresStatus')?.value ?? state.obresStatusFilter ?? ''),
    relation:document.getElementById('v09136ObresRelation')?.value ?? state.obresRelationFilter ?? ''
  };
}

function teimor09136FilterModels(models,filters){
  return models.filter(model=>{
    const queryOk=!filters.q || model.blob.includes(filters.q);
    ...
    const statusOk=!filters.status || strip(model.job.status||'')===filters.status;
    ...
    return queryOk&&yearOk&&clientOk&&statusOk&&relationOk;
  });
}

function teimor09136YearButtons(models,selected){
  const years=[...new Set(models.map(model=>model.year).filter(Boolean))].sort((a,b)=>b-a);
  return '<div class="actions" style="flex-wrap:wrap;margin:8px 0 0"><button class="'+(!selected?'primary':'ghost')+' small" data-v09136-year="">Tots els anys</button>'+years.map(year=>'<button class="'+(String(selected)===String(year)?'primary':'ghost')+' small" data-v09136-year="'+esc(year)+'">'+esc(year)+'</button>').join('')+'</div>';
}

function teimor09136StatusOptions(models,selected){
  const statuses=[...new Set(models.map(model=>model.job.status).filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'ca'));
  return '<option value="">Tots els estats</option>'+statuses.map(status=>'<option value="'+esc(status)+'" '+(strip(status)===strip(selected||'')?'selected':'')+'>'+esc(status)+'</option>').join('');
}

function teimor09136RelationOptions(selected){
  const values=[['','Totes'],['pressupost','Amb pressupost'],['factura','Amb factura'],['completa','Pressupost + factura'],['pendent-factura','Pressupost sense factura'],['sense-documents','Sense documents']];
  return values.map(([value,label])=>'<option value="'+value+'" '+(value===selected?'selected':'')+'>'+label+'</option>').join('');
}

function teimor09136JobRows(models){
  if(!models.length) return '<tr><td colspan="10">'+empty('No hi ha obres que coincideixin amb aquests filtres.')+'</td></tr>';
  return models.map(model=>'<tr data-v09136-obres-row="1"><td>'+esc(model.year||'')+'</td><td><strong>'+esc(model.keyword||'Obra sense paraula clau')+'</strong><br><span class="muted">Adreça obra: '+esc([model.address,model.city].filter(Boolean).join(' · ')||'Pendent de revisar')+'</span></td><td>'+esc(model.client?.name||clientName(model.job.clientId)||'Client pendent')+'</td><td class="num">'+model.budgets.length+'</td><td class="num">'+model.invoices.length+'</td><td class="num">'+model.certifications.length+'</td><td class="num">'+money(model.budgetTotal)+'</td><td class="num">'+money(model.invoiceBase)+'</td><td>'+statusPill(model.job.status||'')+'</td><td class="nowrap"><button class="primary small" data-v0913-trace-job="'+esc(model.job.id)+'" data-v09136-open-job="'+esc(model.job.id)+'">Obrir fitxa</button> <button class="ghost small" data-edit-job="'+esc(model.job.id)+'">Editar obra</button></td></tr>').join('');
}

function teimor09136PageControls(total){
  const size=Number(state.obresPageSize)||40;
  const pages=Math.max(1,Math.ceil(total/size));
  state.obresPage=Math.min(Math.max(1,Number(state.obresPage)||1),pages);
  return '<div class="toolbar" style="margin-top:10px"><span class="small-text">Pàgina '+state.obresPage+' de '+pages+'</span><div class="actions"><button class="ghost small" data-v09136-page="prev" '+(state.obresPage<=1?'disabled':'')+'>← Anterior</button><button class="ghost small" data-v09136-page="next" '+(state.obresPage>=pages?'disabled':'')+'>Següent →</button><label class="small-text">Per pàgina<select id="v09136ObresPageSize"><option '+(size===25?'selected':'')+'>25</option><option '+(size===40?'selected':'')+'>40</option><option '+(size===75?'selected':'')+'>75</option><option '+(size===100?'selected':'')+'>100</option></select></label></div></div>';
}

function teimor09136InvoiceBudgetOptions(model,invoice){
  const budgets=[...model.budgets];
  if(invoice.budgetId && !budgets.some(item=>item.id===invoice.budgetId)){
    const current=byId(data.budgets,invoice.budgetId); if(current) budgets.push(current);
  }
  return '<option value="">Sense pressupost</option>'+budgets.map(item=>'<option value="'+esc(item.id)+'" '+(item.id===invoice.budgetId?'selected':'')+'>'+esc((item.number||item.id)+' · '+(item.title||''))+'</option>').join('');
}

function teimor09136InvoiceJobOptions(model,invoice){
  const ids=[model.job.id,invoice.jobId].filter(Boolean).filter((id,index,array)=>array.indexOf(id)===index);
  return '<option value="">Sense obra</option>'+ids.map(id=>{const job=byId(data.jobs,id)||{};return '<option value="'+esc(id)+'" '+(id===invoice.jobId?'selected':'')+'>'+esc((job.year||'')+' · '+(teimor0913v2JobKeyword(job)||job.title||id))+'</option>';}).join('');
}

function teimor09136SummaryTab(model){
  const client=model.client||{};
  return '<div class="grid four"><div class="kpi"><span>Pressupostos</span><strong>'+model.budgets.length+'</strong></div><div class="kpi"><span>Factures</span><strong>'+model.invoices.length+'</strong></div><div class="kpi"><span>Certificacions</span><strong>'+model.certifications.length+'</strong></div><div class="kpi"><span>Pressupostat s/IVA</span><strong>'+money(model.budgetTotal)+'</strong></div></div><div class="grid two"><div class="card"><h3>Identificació</h3><p><strong>Paraula clau / referència:</strong> '+esc(model.keyword||'Pendent')+'<br><strong>Adreça de l’obra:</strong> '+esc([model.address,model.city].filter(Boolean).join(' · ')||'Pendent')+'<br><strong>Any:</strong> '+esc(model.year||'')+'<br><strong>Estat:</strong> '+esc(model.job.status||'')+'</p></div><div class="card"><h3>Client i dades fiscals</h3><p><strong>'+esc(client.name||'Client pendent')+'</strong><br>NIF/DNI/CIF: '+esc(client.nif||'')+'<br>'+esc([client.fiscalAddress,client.postalCode,client.city].filter(Boolean).join(' · ')||'Dades fiscals pendents')+'</p></div></div><div class="card notice-blue"><strong>Traçabilitat:</strong> aquesta fitxa agrupa els documents per l’obra consolidada. Si una factura surt aquí, es pot revisar o canviar la seva relació des de la pestanya «Factures».</div>';
}
...
function teimor09136InvoicesTab(model){
  if(!model.invoices.length) return '<div class="card">'+empty('Aquesta obra encara no té cap factura vinculada.')+'<button class="primary small" data-go="invoices">Anar a Factures</button></div>';
  const rows=model.invoices.map(invoice=>'<tr><td>'+esc(dateDisplay(invoice.date))+'</td><td><strong>'+esc(invoice.number||invoice.id)+'</strong></td><td>'+esc(invoice.concept||'')+'</td><td><select data-v09136-invoice-job="'+esc(invoice.id)+'">'+teimor09136InvoiceJobOptions(model,invoice)+'</select></td><td><select data-v09136-invoice-budget="'+esc(invoice.id)+'">'+teimor09136InvoiceBudgetOptions(model,invoice)+'</select></td><td class="num">'+money(invoiceBase(invoice))+'</td><td class="num">'+money(invoiceTotal(invoice))+'</td><td class="nowrap"><button class="primary small" data-v09136-save-invoice="'+esc(invoice.id)+'">Guardar relació</button> <button class="ghost small" data-edit-invoice="'+esc(invoice.id)+'">Editar</button> <button class="ghost small" data-v09134-invoice-pdf="'+esc(invoice.id)+'">Veure PDF</button> <button class="ghost small" data-v09132-download-invoice="'+esc(invoice.id)+'">Descarregar factura</button></td></tr>');
  return '<div class="card"><div class="toolbar"><div><h3>Factures vinculades</h3><p class="muted">Pots canviar l’obra o el pressupost de cada factura i guardar la relació.</p></div><button class="ghost small" data-go="invoices">Veure totes les factures</button></div>'+table(['Data','Número','Concepte','Obra','Pressupost','Base','Total','Acció'],rows)+'</div>';
}
...
function teimor09136TimeMaterialsTab(model){
  const timeRows=[...(model.hours||[])];
  const agendaRows=[...(model.agenda||[])];
  const materials=(model.materials||[]).slice(0,150);
  const timeHtml=timeRows.length?table(['Data','Descripció','Hores','Cost','Estat'],timeRows.map(item=>'<tr><td>'+esc(dateDisplay(item.date))+'</td><td>'+esc(item.concept||item.title||item.notes||'')+'</td><td class="num">'+esc(item.hours||item.qty||'')+'</td><td class="num">'+money(item.total||item.cost||num(item.hours)*num(item.unitPrice))+'</td><td>'+esc(item.status||'')+'</td></tr>')):empty('Encara no hi ha repercussió d’hores registrada.');
  const agendaHtml=agendaRows.length?table(['Data','Hora','Tipus','Nota','Estat'],agendaRows.map(item=>'<tr><td>'+esc(dateDisplay(item.date))+'</td><td>'+esc(item.time||'')+'</td><td>'+esc(item.type||'')+'</td><td>'+esc(item.title||item.notes||'')+'</td><td>'+esc(item.status||'')+'</td></tr>')):empty('No hi ha notes de temps o agenda vinculades.');
  const materialHtml=materials.length?table(['Origen','Material','Ut','Quantitat','Preu/ut','Total'],materials.map(item=>'<tr><td>'+esc(item.sourceBudget||item.budgetId||'')+'</td><td>'+esc(item.concept||item.name||'')+'</td><td>'+esc(item.unit||'')+'</td><td class="num">'+esc(item.qty||item.quantity||'')+'</td><td class="num">'+money(item.unitPrice||item.price)+'</td><td class="num">'+money(item.total||lineTotal(item))+'</td></tr>')):empty('Encara no hi ha materials o partides de material separats.');
  return '<div class="grid two"><div class="card"><div class="toolbar"><h3>Temps / hores</h3><button class="ghost small" data-go="agenda">Obrir agenda</button></div>'+timeHtml+'</div><div class="card"><h3>Notes i seguiment de temps</h3>'+agendaHtml+'</div></div><div class="card"><h3>Materials i repercussió</h3>'+materialHtml+'</div>';
}

function teimor09136PerformanceTab(model){
  const difference=model.invoiceBase-model.budgetTotal;
  const percentage=model.budgetTotal?difference/model.budgetTotal*100:0;
  return '<div class="grid four"><div class="kpi"><span>Pressupost s/IVA</span><strong>'+money(model.budgetTotal)+'</strong></div><div class="kpi"><span>Facturat s/IVA</span><strong>'+money(model.invoiceBase)+'</strong></div><div class="kpi '+(difference>=0?'good':'bad')+'"><span>Diferència</span><strong>'+money(difference)+'</strong></div><div class="kpi"><span>Diferència %</span><strong>'+percentage.toFixed(2)+'%</strong></div></div><div class="card"><h3>Rendiment per pressupost</h3>'+table(['Pressupost','Base pressupost','Facturat associat','Diferència'],model.budgets.map(budget=>{const invoices=model.invoices.filter(item=>item.budgetId===budget.id);const billed=invoices.reduce((sum,item)=>sum+invoiceBase(item),0);return '<tr><td>'+esc(budget.number||budget.id)+'</td><td class="num">'+money(budgetBase(budget))+'</td><td class="num">'+money(billed)+'</td><td class="num">'+money(billed-budgetBase(budget))+'</td></tr>';}))+'</div><div class="card notice-blue">El rendiment es calcula amb les dades de pressupostos i factures que hi ha actualment vinculades. Les hores i materials es poden completar a la pestanya corresponent.</div>';
}

function teimor09136NotesTab(model){
  return '<div class="card"><h3>Notes de l’obra</h3><textarea id="v09136JobNotes" style="min-height:180px;width:100%">'+esc(model.job.notes||'')+'</textarea><div class="actions" style="margin-top:10px"><button class="primary" data-v09136-save-notes="'+esc(model.job.id)+'">Guardar notes</button></div></div>';
}

function teimor09136JobFicha(model){
  const tabs=[['resum','Resum'],['pressupost','Pressupostos'],['factures','Factures'],['certificacions','Certificacions'],['documentacio','Documentació'],['albarans','Albarans'],['notes','Notes'],['temps','Temps i materials'],['rendiment','Rendiment']];
  const tab= tabs.some(item=>item[0]===state.obresJobTab)?state.obresJobTab:'resum';
  let body=teimor09136SummaryTab(model);
  if(tab==='pressupost') body=teimor09136BudgetsTab(model);
  if(tab==='factures') body=teimor09136InvoicesTab(model);
  if(tab==='certificacions') body='<div class="card">'+(model.certifications.length?teimor0913TraceCertificationHtml(model.certifications):empty('Aquesta obra encara no té certificacions vinculades.'))+'</div>';
  if(tab==='documentacio') body=teimor09136DocumentsTab(model,false);
  if(tab==='albarans') body=teimor09136DocumentsTab(model,true);
  if(tab==='notes') body=teimor09136NotesTab(model);
  if(tab==='temps') body=teimor09136TimeMaterialsTab(model);
  if(tab==='rendiment') body=teimor09136PerformanceTab(model);
  return '<div class="card" id="v09136JobFicha"><div class="toolbar"><div><h2>Fitxa d’obra: '+esc(model.keyword||'Obra sense nom')+'</h2><p class="muted">'+esc(clientName(model.job.clientId)||'Client pendent')+' · '+esc([model.address,model.city].filter(Boolean).join(' · ')||'Adreça pendent')+'</p></div><div class="actions"><button class="ghost" data-edit-job="'+esc(model.job.id)+'">Editar obra</button><button class="ghost" data-v09136-close-job>Tancar fitxa</button></div></div><div class="job-detail-tabs">'+tabs.map(item=>'<button class="'+(tab===item[0]?'active':'')+'" data-v09136-job-tab="'+item[0]+'">'+item[1]+'</button>').join('')+'</div><div style="margin-top:14px">'+body+'</div></div>';
}

function teimor09136RenderObres(prepareData=true){
  if(prepareData) teimor0913EnsureData();
  data.meta=data.meta||{};
  data.meta.version='9.13.6-obres-filtres-fitxa-rapida';
  setHeader('Obres / fitxes de treball','Filtra per any, client, obra i vinculació; obre la fitxa completa per revisar pressupostos, factures i documents.');
  const built=teimor09136BuildModels();
  state.obresModels=built.models;
  const filters=teimor09136ObresFilters();
  state.obresSearch=filters.q; state.obresYearFilter=filters.year; state.obresClientFilter=filters.client; state.obresStatusFilter=filters.status; state.obresRelationFilter=filters.relation;
  const filtered=teimor09136FilterModels(built.models,filters);
  const size=Number(state.obresPageSize)||40;
  const pages=Math.max(1,Math.ceil(filtered.length/size));
  state.obresPage=Math.min(Math.max(1,Number(state.obresPage)||1),pages);
  const pageRows=filtered.slice((state.obresPage-1)*size,state.obresPage*size);
  const selected=state.selectedJobId?built.models.find(model=>model.job.id===state.selectedJobId):null;
  const allBudgetTotal=(data.budgets||[]).reduce((sum,item)=>sum+budgetBase(item),0);
  const allInvoiceTotal=(data.invoices||[]).reduce((sum,item)=>sum+invoiceBase(item),0);
  const usedClientIds=new Set(built.models.map(model=>model.job.clientId).filter(Boolean));
  const visibleClients=data.clients.filter(client=>usedClientIds.has(client.id));
  const clientOptions='<option value="">Tots els clients</option>'+options(visibleClients,state.obresClientFilter||'',client=>client.name||client.id);
  const filterInfo='Mostrant '+(filtered.length?((state.obresPage-1)*size+1)+'–'+Math.min(state.obresPage*size,filtered.length):0)+' de '+filtered.length+' obres filtrades · total real: '+built.models.length+' obres · '+data.budgets.length+' pressupostos · '+data.invoices.length+' factures.';
  const detail=selected?teimor09136JobFicha(selected):'<div class="card notice-blue"><h2>Obre una obra per veure la fitxa completa</h2><p>La fitxa conté pressupostos, factures, certificacions, documentació, albarans, notes, temps/materials i rendiment.</p></div>';
  setContent('<div class="grid four"><div class="kpi"><span>Obres totals</span><strong>'+built.models.length+'</strong></div><div class="kpi"><span>Obres filtrades</span><strong>'+filtered.length+'</strong></div><div class="kpi"><span>Pressupostos</span><strong>'+data.budgets.length+'</strong></div><div class="kpi"><span>Factures</span><strong>'+data.invoices.length+'</strong></div></div><div class="card"><div class="toolbar"><div><h2>Llistat d’obres</h2><p class="muted">Pressupostat: '+money(allBudgetTotal)+' · Facturat s/IVA: '+money(allInvoiceTotal)+'</p></div><div class="actions"><button class="ghost" data-v09136-clear-filters>Netejar filtres</button><button class="primary" data-go="importer">Importar factures / certificacions</button></div></div><div class="filter-grid"><label>Cerca obra / client / adreça<input id="v09136ObresSearch" placeholder="RCP, Major 10, coberta..." value="'+esc(state.obresSearch||'')+'"></label><label>Any<select id="v09136ObresYear"><option value="">Tots els anys</option>'+[...new Set(built.models.map(model=>model.year).filter(Boolean))].sort((a,b)=>b-a).map(year=>'<option value="'+esc(year)+'" '+(String(year)===String(state.obresYearFilter||'')?'selected':'')+'>'+esc(year)+'</option>').join('')+'</select></label><label>Client<select id="v09136ObresClient">'+clientOptions+'</select></label><label>Estat<select id="v09136ObresStatus">'+teimor09136StatusOptions(built.models,state.obresStatusFilter||'')+'</select></label><label>Vinculació<select id="v09136ObresRelation">'+teimor09136RelationOptions(state.obresRelationFilter||'')+'</select></label></div>'+teimor09136YearButtons(built.models,state.obresYearFilter||'')+'<div id="v09136ObresFilterInfo" class="small-text" style="margin:10px 0">'+filterInfo+'</div><div class="table-wrap"><table><thead><tr><th>Any</th><th>Identificació de l’obra</th><th>Client</th><th>Pressupostos</th><th>Factures</th><th>Certificacions</th><th>Pressupostat</th><th>Facturat</th><th>Estat</th><th>Accions</th></tr></thead><tbody>'+teimor09136JobRows(pageRows)+'</tbody></table></div>'+teimor09136PageControls(filtered.length)+'</div>'+detail);
  teimor09136BindObresUi();
}

function teimor09136SaveInvoiceRelation(invoiceId,row){
  const invoice=byId(data.invoices,invoiceId); if(!invoice) return;
  const jobId=row.querySelector('[data-v09136-invoice-job]')?.value||'';
  const budgetId=row.querySelector('[data-v09136-invoice-budget]')?.value||'';
  const budget=budgetId?byId(data.budgets,budgetId):null;
  invoice.budgetId=budget?.id||'';
  invoice.jobId=budget?.jobId||jobId||'';
  if(budget?.clientId) invoice.clientId=budget.clientId;
  else if(invoice.jobId) invoice.clientId=byId(data.jobs,invoice.jobId)?.clientId||invoice.clientId||'';
  invoice.matchStatus=invoice.jobId||invoice.budgetId?'manual':'pendent';
  invoice.matchConfidence=invoice.jobId||invoice.budgetId?100:0;
  invoice.matchReason=invoice.jobId||invoice.budgetId?'Relació revisada des de la fitxa d’obra.':'Sense obra ni pressupost vinculats.';
  invoice.status=invoice.jobId||invoice.budgetId?'Importada · vinculada':'Importada · pendent de revisar';
  saveData();
  teimor09134RepairData();
  teimor09136RenderObres(false);
}

function teimor09136RenderKeepingFocus(inputId){
  ...
  if(next){
    next.focus();
    if(typeof start==='number' && typeof next.setSelectionRange==='function') next.setSelectionRange(start,end??start);
  }
}

function teimor09136BindObresUi(){
  const rerender=()=>{ state.obresPage=1; teimor09136RenderObres(false); };
  const search=document.getElementById('v09136ObresSearch'); if(search) search.oninput=event=>{state.obresSearch=event.target.value||'';state.obresPage=1;teimor09136RenderKeepingFocus('v09136ObresSearch');};
  const year=document.getElementById('v09136ObresYear'); if(year) year.onchange=event=>{state.obresYearFilter=event.target.value||'';rerender();};
  const client=document.getElementById('v09136ObresClient'); if(client) client.onchange=event=>{state.obresClientFilter=event.target.value||'';rerender();};
  const status=document.getElementById('v09136ObresStatus'); if(status) status.onchange=event=>{state.obresStatusFilter=event.target.value||'';rerender();};
  const relation=document.getElementById('v09136ObresRelation'); if(relation) relation.onchange=event=>{state.obresRelationFilter=event.target.value||'';rerender();};
  const pageSize=document.getElementById('v09136ObresPageSize'); if(pageSize) pageSize.onchange=event=>{state.obresPageSize=Number(event.target.value)||40;state.obresPage=1;teimor09136RenderObres(false);};
  document.querySelectorAll('[data-v09136-year]').forEach(button=>button.onclick=event=>{event.preventDefault();state.obresYearFilter=button.dataset.v09136Year||'';state.obresPage=1;teimor09136RenderObres(false);});
  document.querySelectorAll('[data-v09136-page]').forEach(button=>button.onclick=event=>{event.preventDefault();state.obresPage+=(button.dataset.v09136Page==='next'?1:-1);teimor09136RenderObres(false);});
  document.querySelectorAll('[data-v09136-open-job],[data-v0913-trace-job]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();const id=button.dataset.v09136OpenJob||button.dataset.v0913TraceJob;state.selectedJobId=id;state.obresJobTab='resum';teimor09136RenderObres(false);});
  document.querySelectorAll('[data-edit-job]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();teimor09135OpenJobEditor(button.dataset.editJob);});
  document.querySelectorAll('[data-v09136-job-tab]').forEach(button=>button.onclick=event=>{event.preventDefault();state.obresJobTab=button.dataset.v09136JobTab||'resum';teimor09136RenderObres(false);});
  document.querySelectorAll('[data-v09136-close-job]').forEach(button=>button.onclick=event=>{event.preventDefault();state.selectedJobId='';teimor09136RenderObres(false);});
  document.querySelectorAll('[data-v09136-clear-filters]').forEach(button=>button.onclick=event=>{event.preventDefault();state.obresSearch='';state.obresYearFilter='';state.obresClientFilter='';state.obresStatusFilter='';state.obresRelationFilter='';state.obresPage=1;teimor09136RenderObres(false);});
  document.querySelectorAll('[data-v09136-save-notes]').forEach(button=>button.onclick=event=>{event.preventDefault();const job=byId(data.jobs,button.dataset.v09136SaveNotes);const notes=document.getElementById('v09136JobNotes');if(job&&notes){job.notes=notes.value||'';saveData();teimor09136RenderObres(false);}});
  document.querySelectorAll('[data-v09136-save-invoice]').forEach(button=>button.onclick=event=>{event.preventDefault();const row=button.closest('tr');teimor09136SaveInvoiceRelation(button.dataset.v09136SaveInvoice,row);});
}

renderObresV0913=function(){
  try{ teimor09136RenderObres(); }
  catch(error){ console.error('No s’ha pogut obrir la nova fitxa d’obra:',error); teimor09132RenderObresFallback(error); }
};

/* Obre la vista nova també des de qualsevol acció antiga que encara cridi
   directament renderObresV0913 o el mode segur de V09.13. */
...
/* =========================================================
   ...
   - No crea obres en relacionar documents.
   - Consolida obres duplicades després d’enriquir-les amb les dades dels
     pressupostos i factures que ja hi estan vinculats.
   - Fa que una factura vinculada a un pressupost aparegui també dins l’obra.
   - Reforça els botons de traçabilitat/edició.
   - Imprimeix referències comercials, no missatges interns de la aplicació.
   ========================================================= */

data.meta = data.meta || {};
data.meta.version = '9.13.5-relacio-traçabilitat-pdf';

function teimor09135Clean(value){
  return String(value ?? '').replace(/\s+/g,' ').trim();
}

function teimor09135GenericKeyword(value){
  const text=teimor0913Norm(value||'');
  return !text || /^(?:concepte pendent de revisar|concepte pendent|obra sense nom|feina sense nom|pendent de revisar|factura importada|certificacio importada|certificació importada)$/.test(text);
}

function teimor09135BadAddress(value){
  const text=teimor09135Clean(value);
  if(!text) return false;
  if(text.length>180 || /[\n\r]/.test(String(value||''))) return true;
  return /\b(?:partid(?:a|es)|unitat(?:s)?|quantitat|preu|import|descripci[oó]|concepte|treballs|total|iva|base)\b/i.test(text);
}

function teimor09135SetIfBetter(target,field,value,{replaceBad=false}={}){
  const next=teimor09135Clean(value);
  if(!next) return false;
  const current=teimor09135Clean(target[field]);
  if(!current || (replaceBad && teimor09135BadAddress(current) && !teimor09135BadAddress(next))){
    target[field]=next;
    return true;
  }
  return false;
...
function teimor09135EnrichJob(job,source={},budget=null){
  if(!job) return false;
  const snap=source.clientSnapshot||{};
  let changed=false;
  const clientId=budget?.clientId||source.clientId||'';
  if(!job.clientId && clientId){ job.clientId=clientId; changed=true; }

  const sourceConcept=teimor09135Clean(source.concept);
  const usableConcept=/^(?:factura|certificaci[oó])/i.test(sourceConcept)?'':sourceConcept;
  const keyword=budget?.keyword||budget?.workKeyword||budget?.title||source.keyword||source.workKeyword||usableConcept||'';
  if(teimor09135GenericKeyword(job.keyword||job.title) && !teimor09135GenericKeyword(keyword)){
    job.keyword=teimor09135Clean(keyword);
    job.title=job.keyword;
    changed=true;
  }else if(!job.keyword && !teimor09135GenericKeyword(keyword)){
    job.keyword=teimor09135Clean(keyword);
    changed=true;
  }
  if(!job.title && job.keyword){ job.title=job.keyword; changed=true; }

  const address=budget?.workAddress||budget?.address||source.workAddress||source.address||snap.workAddress||'';
  if(teimor09135SetIfBetter(job,'workAddress',address,{replaceBad:true})){ changed=true; }
  if(!job.address || teimor09135BadAddress(job.address)){
    const next=teimor09135Clean(job.workAddress||address);
    if(next && job.address!==next){ job.address=next; changed=true; }
  }
  const city=budget?.workCity||budget?.city||source.workCity||source.workCity||snap.workCity||'';
  if(teimor09135SetIfBetter(job,'workCity',city)){ changed=true; }
  if(!job.city && job.workCity){ job.city=job.workCity; changed=true; }
  const postal=budget?.workPostalCode||budget?.postalCode||source.workPostalCode||snap.workPostalCode||'';
  if(teimor09135SetIfBetter(job,'workPostalCode',postal)){ changed=true; }
  if(!job.postalCode && job.workPostalCode){ job.postalCode=job.workPostalCode; changed=true; }
  if(!job.year){
    const year=Number(String(budget?.date||source.date||'').slice(0,4));
    if(year){ job.year=year; changed=true; }
  }
  return changed;
}

function teimor09135EnrichJobsFromDocuments(){
  let changed=false;
  (data.budgets||[]).forEach(budget=>{
    const job=budget.jobId?byId(data.jobs,budget.jobId):null;
    if(job) changed=teimor09135EnrichJob(job,{},budget)||changed;
  });
  [...(data.invoices||[]),...(data.certifications||[])].forEach(doc=>{
    let budget=doc.budgetId?byId(data.budgets,doc.budgetId):null;
    if(!budget && doc.legacyBudgetId) budget=byId(data.budgets,doc.legacyBudgetId);
    if(budget && !budget.jobId && doc.jobId && byId(data.jobs,doc.jobId)){
      budget.jobId=doc.jobId;
      changed=true;
    }
    const job=doc.jobId?byId(data.jobs,doc.jobId):(budget?.jobId?byId(data.jobs,budget.jobId):null);
    if(job) changed=teimor09135EnrichJob(job,doc,budget)||changed;
  });
  return changed;
}

function teimor09135PropagateRelations(){
  let changed=false;
  const documents=[...(data.invoices||[]),...(data.certifications||[])];
  documents.forEach(doc=>{
    if(!doc.budgetId && doc.matchStatus==='automatica' && Array.isArray(doc.candidateBudgetIds) && doc.candidateBudgetIds.length===1 && byId(data.budgets,doc.candidateBudgetIds[0])){
      doc.budgetId=doc.candidateBudgetIds[0];
      changed=true;
    }
    if(doc.legacyBudgetId && !doc.budgetId && byId(data.budgets,doc.legacyBudgetId)){
      doc.budgetId=doc.legacyBudgetId;
      changed=true;
    }
    if(doc.budgetId){
      const budget=byId(data.budgets,doc.budgetId);
      if(!budget){
        ...
        if(!budget.jobId && doc.jobId && byId(data.jobs,doc.jobId)){
          budget.jobId=doc.jobId;
          changed=true;
        }
        if(budget.jobId && byId(data.jobs,budget.jobId) && doc.jobId!==budget.jobId){
          doc.jobId=budget.jobId;
          changed=true;
        }
        if(budget.clientId && doc.clientId!==budget.clientId){
          doc.clientId=budget.clientId;
          changed=true;
        }
      }
    }
    if(doc.jobId && !byId(data.jobs,doc.jobId)){
      doc.jobId='';
      changed=true;
    }
    if(doc.jobId){
      const job=byId(data.jobs,doc.jobId);
      if(job && job.clientId && !doc.clientId){
        doc.clientId=job.clientId;
        changed=true;
      }
    }
  });
...
function teimor09135JobKey(job){
  const client=teimor0913NormId(job?.clientId||'');
  const rawAddress=job?.workAddress||job?.address||'';
  const address=teimor09135BadAddress(rawAddress)?'':teimor09132AddressText(rawAddress);
  const keyword=teimor0913Norm(job?.keyword||job?.title||'');
  const generic=teimor09135GenericKeyword(keyword);
  if(client && address) return 'client|'+client+'|'+address;
  if(address && !generic && keyword) return 'address|'+address+'|'+keyword;
  if(client && !generic && keyword) return 'client|'+client+'|'+keyword+'|'+String(job?.year||'');
  return '';
}

/* El reparador V09.13.4 continua sent la base; aquí només hi afegim les
   dades que necessita per consolidar i propagar relacions correctament. */
const __teimor09134RepairDataV09135=teimor09134RepairData;
teimor09134JobKey=teimor09135JobKey;
teimor09134RepairData=function(){
  data.meta=data.meta||{};
  data.jobs=Array.isArray(data.jobs)?data.jobs:[];
  data.budgets=Array.isArray(data.budgets)?data.budgets:[];
  data.invoices=Array.isArray(data.invoices)?data.invoices:[];
  ...
  data.attachments=Array.isArray(data.attachments)?data.attachments:[];
  data.agenda=Array.isArray(data.agenda)?data.agenda:[];
  let changed=teimor09135EnrichJobsFromDocuments();
  const base=__teimor09134RepairDataV09135();
  changed=base.changed||changed;
  const propagated=teimor09135PropagateRelations();
  changed=propagated||changed;

  const budgetJobIds=new Set((data.budgets||[]).map(item=>item.jobId).filter(Boolean));
  const referenced=new Set(budgetJobIds);
  [...(data.invoices||[]),...(data.certifications||[]),...(data.attachments||[]),...(data.agenda||[])].forEach(item=>{
    if(item.jobId) referenced.add(item.jobId);
    if(item.budgetId){ const budget=byId(data.budgets,item.budgetId); if(budget?.jobId) referenced.add(budget.jobId); }
  });
  const beforeJobs=data.jobs.length;
  data.jobs=data.jobs.filter(job=>{
    if(referenced.has(job.id) || job.mainBudgetId && byId(data.budgets,job.mainBudgetId)) return true;
    const marker=[job.source,job.sourceFile,job.importedAt,job.clientTempKey,job.jobTempKey,job.notes].filter(Boolean).join(' ');
    return !/import|excel|hist[oò]ric|pendent de revisar|factura|certificaci[oó]/i.test(marker);
  });
  if(data.jobs.length!==beforeJobs) changed=true;

  const summary={...(data.meta.v09134RepairSummary||{}),v09135JobsRemoved:beforeJobs-data.jobs.length,remainingJobs:data.jobs.length,relationsPropagated:propagated};
  if(JSON.stringify(data.meta.v09135RepairSummary)!==JSON.stringify(summary)){
    data.meta.v09135RepairSummary=summary;
    changed=true;
  }
  return {changed,summary};
};

/* Una factura pot tenir només budgetId; en aquest cas també és de l’obra del
   pressupost, encara que el fitxer antic no hagués guardat jobId. */
const __teimorBaseJobInvoicesV09135=jobInvoices;
jobInvoices=function(jobId){
  const budgetIds=new Set((data.budgets||[]).filter(item=>item.jobId===jobId).map(item=>item.id));
  const rows=(data.invoices||[]).filter(item=>item.jobId===jobId || (item.budgetId && budgetIds.has(item.budgetId)));
  const seen=new Set();
  return rows.filter(item=>{ if(seen.has(item.id)) return false; seen.add(item.id); return true; });
};

function teimor09135OpenTrace(jobId){
  if(!jobId || !byId(data.jobs,jobId)) return alert('No s’ha trobat aquesta obra.');
  state.view='obres';
  state.selectedJobId=jobId;
  render();
}

function teimor09135OpenJobEditor(jobId){
  if(!jobId || !byId(data.jobs,jobId)) return alert('No s’ha trobat aquesta obra.');
  state.view='jobs';
  state.selectedJobId=jobId;
  renderJobs(jobId);
...
teimor09133BindObresActions=function(){
  document.querySelectorAll('[data-v0913-trace-job]').forEach(button=>button.onclick=event=>{
    event.preventDefault();
    event.stopPropagation();
    teimor09135OpenTrace(button.dataset.v0913TraceJob);
  });
  document.querySelectorAll('[data-edit-job]').forEach(button=>button.onclick=event=>{
    event.preventDefault();
    event.stopPropagation();
    teimor09135OpenJobEditor(button.dataset.editJob);
  });
  document.querySelectorAll('[data-v09133-auto-relate],[data-v09134-auto-relate]').forEach(button=>button.onclick=event=>{
    event.preventDefault();
    event.stopPropagation();
    teimor09133RelateAllPending();
  });
  const content=document.getElementById('content');
  if(content && content.dataset.teimor09135Delegation!=='1'){
    content.dataset.teimor09135Delegation='1';
    content.addEventListener('click',event=>{
      const trace=event.target.closest?.('[data-v0913-trace-job]');
      if(trace){ event.preventDefault(); teimor09135OpenTrace(trace.dataset.v0913TraceJob); return; }
      const edit=event.target.closest?.('[data-edit-job]');
      if(edit){ event.preventDefault(); teimor09135OpenJobEditor(edit.dataset.editJob); }
    });
  }
};

teimor09133RelateAllPending=function(){
  teimor0913EnsureData();
  teimor09134RepairData();
  const beforeJobs=data.jobs.length;
  const documents=[...(data.invoices||[]),...(data.certifications||[])];
  let checked=0,linked=0,unresolved=0;
  documents.forEach(doc=>{
    const already=!!(doc.jobId&&doc.budgetId);
    if(!already) checked++;
    if(doc.matchStatus!=='manual' && !doc.budgetId && !doc.jobId && typeof teimor0913ApplyMatch==='function') teimor0913ApplyMatch(doc);
    if(doc.budgetId){
      const budget=byId(data.budgets,doc.budgetId);
      if(budget){
        if(budget.jobId) doc.jobId=budget.jobId;
        if(budget.clientId) doc.clientId=budget.clientId;
      }
    }
    if(doc.jobId && byId(data.jobs,doc.jobId)){
      doc.status='Importada · vinculada';
      linked++;
    }else if(doc.budgetId) linked++;
    else unresolved++;
  });
  teimor09135PropagateRelations();
  const afterRepair=teimor09134RepairData();
  saveData();
  const removed=beforeJobs-data.jobs.length;
  alert(`Relació massiva acabada. Documents revisats: ${checked}. Vinculats: ${linked}. Pendents de validar: ${unresolved}. Obres consolidades: ${removed>0?removed:0}. Total d’obres actual: ${data.jobs.length}.`);
  state.view='obres';
  render();
};

function teimor09135InvoiceBudget(invoice){
  return byId(data.budgets,invoice?.budgetId)||byId(data.budgets,invoice?.legacyBudgetId)||null;
}

function teimor09135InvoiceJob(invoice,budget){
  return byId(data.jobs,invoice?.jobId)||byId(data.jobs,budget?.jobId)||null;
}

function teimor09135PrintableNotes(invoice){
  return String(invoice?.notes||'').split(/\r?\n/).map(line=>line.trim()).filter(Boolean).filter(line=>!/(registre separat automàticament|relaci[oó] corregida|relaci[oó] automàtica|fitxer original pendent|importada automàticament|pendent de revisar la coincid[eè]ncia)/i.test(line)).join('\n');
}

openInvoicePreview=function(id){
  const invoice=byId(data.invoices,id);
  if(!invoice) return alert('No s’ha trobat aquesta factura.');
  const budget=teimor09135InvoiceBudget(invoice)||{};
  const job=teimor09135InvoiceJob(invoice,budget)||{};
  const client=byId(data.clients,invoice.clientId)||{};
  const snapshot=invoice.clientSnapshot||{};
  const s=data.settings.contractista||{};
  const name=client.name||snapshot.name||'Client pendent de revisar';
  const nif=client.nif||snapshot.nif||'';
  const fiscal=[client.fiscalAddress||snapshot.fiscalAddress,client.postalCode||snapshot.postalCode,client.city||snapshot.city].filter(Boolean).join(' · ');
  const workAddress=snapshot.workAddress||job.workAddress||job.address||budget.workAddress||'';
  const workCity=snapshot.workCity||job.workCity||job.city||budget.workCity||'';
  const workKeyword=job.keyword||job.title||budget.keyword||budget.title||'';
  const workReference=[workKeyword,workAddress,workCity].filter(Boolean).join(' · ')||'Pendent de revisar';
  const budgetReference=budget.number||budget.oldNumber||invoice.budgetReference||'No assignat';
  const concept=invoice.concept||'Factura importada';
  const notes=teimor09135PrintableNotes(invoice);
  const html='<div class="preview-toolbar actions"><button class="primary" onclick="window.print()">Imprimir / guardar PDF</button><button class="ghost" onclick="window.close()">Tancar</button></div>'+
    '<div class="a4-sheet"><div class="preview-colorbar"></div>'+
    '<div class="preview-header"><div><h1>'+esc(s.name||'TEIMOR')+'</h1><p>'+esc(s.nif||'')+'<br>'+esc(s.address||'')+'<br>'+esc(s.city||'')+'<br>'+esc(s.phone||'')+'</p></div>'+
    '<div class="client-box"><span>Client</span><strong>'+esc(name)+'</strong><br>'+esc(nif)+'<br>'+esc(fiscal)+'</div></div>'+
    '<div class="preview-meta"><div><strong>Data:</strong> '+dateDisplay(invoice.date)+'</div><div><strong>Factura núm.:</strong> '+esc(invoice.number||invoice.id)+'</div><div><strong>Estat:</strong> '+esc(invoice.paid?'Pagada':(invoice.status||'Pendent'))+'</div></div>'+
    '<div style="border:1px solid #fed7aa;border-left:5px solid #c2410c;background:#fff7ed;padding:8px 10px;margin:0 0 8mm;font-size:12px"><div><strong>Referència obra:</strong> '+esc(workReference)+'</div><div><strong>Segons pressupost:</strong> '+esc(budgetReference)+'</div></div>'+
    '<h2>'+esc(concept)+'</h2>'+
    (workAddress||workCity?'<p><strong>Obra:</strong> '+esc([workAddress,workCity].filter(Boolean).join(' · '))+'</p>':'')+
    '<table class="preview-table"><thead><tr><th>Concepte / descripció</th><th>Base s/IVA</th><th>IVA</th><th>Total</th></tr></thead><tbody><tr><td>'+esc(concept)+(notes?'<div class="preview-desc">'+esc(notes)+'</div>':'')+'</td><td class="num">'+money(invoiceBase(invoice))+'</td><td class="num">'+num(invoice.iva).toFixed(2)+'%</td><td class="num">'+money(invoiceTotal(invoice))+'</td></tr></tbody></table>'+
    '<div class="preview-totals"><div>Base s/IVA: <strong>'+money(invoiceBase(invoice))+'</strong></div><div>IVA '+num(invoice.iva).toFixed(2)+'%: <strong>'+money(invoiceTotal(invoice)-invoiceBase(invoice))+'</strong></div><div>Total: <strong>'+money(invoiceTotal(invoice))+'</strong></div></div>'+
    (notes?'<div class="preview-notes">'+esc(notes)+'</div>':'')+'</div>';
  if(typeof window==='undefined' || typeof window.open!=='function') return alert('El navegador no permet obrir la vista PDF.');
  const w=window.open('', '_blank');
  if(!w) return alert('El navegador ha bloquejat la finestra de previsualització. Permet pop-ups per aquesta app.');
  w.document.write('<!doctype html><html lang="ca"><head><meta charset="utf-8"><title>Factura '+esc(invoice.number||invoice.id)+'</title><style>'+previewCss()+'</style></head><body>'+html+'</body></html>');
...
/* =========================================================
   TEIMOR V09.13.9 · FITXA AUTOCONTINGUDA I FLUX DE TREBALL
   - La pestanya activa sempre queda marcada al menú lateral.
   - L’obra s’edita en un modal superior, amb Guardar i Cancel·lar fiables.
   - Pressupost, factura/certificació, albarà, hores i materials es treballen
     des de la fitxa, sense redirigir a les pantalles principals.
   - Les factures noves neixen com a proforma i conserven la relació amb obra
     i pressupost; la numeració nova és correlativa per any i tipus.
   ========================================================= */
(function(){
  const VERSION='9.13.9-fitxa-autocontinguda-flux-obra';

  function text(value){ return String(value??'').replace(/\s+/g,' ').trim(); }
  function safeArray(value){ return Array.isArray(value)?value:[]; }
  function currentJob(jobId){ return byId(data.jobs,jobId||state.selectedJobId)||null; }
  function currentReturnJob(){ return state.v09139ReturnJobId || state.selectedJobId || ''; }
  function hasOverlay(){ return !!document.getElementById('v09137JobOverlay'); }
  function cleanKeyword(value){
    let result=text(value);
    result=result.replace(/^\s*[.*•·-]\s*/,'');
    result=result.replace(/^(?:pressupost|presupuesto|obra|feina|treball|trabajo|concepte|concepto)\s*(?:n[úu]m(?:ero)?\.?\s*[:#-]?\s*[\w/-]+\s*[-:])?\s*/i,'');
    result=result.replace(/\s*[-–—:]\s*$/,'').trim();
    if(result.length>110){
      const first=result.split(/[.;!?]\s+/)[0];
      result=(first||result).slice(0,110).trim();
    }
    return result;
  }
  function genericKeyword(value){
    const result=strip(value||'');
    return !result || /^(?:obra|feina|obra sense nom|feina sense nom|concepte pendent(?: de revisar)?|pendent de revisar|pressupost importat|factura importada|certificaci[oó] importada)$/.test(result);
  }
  function badWorkAddress(value){
    const result=text(value);
    if(!result) return true;
    return result.length>180 || /\b(?:partid(?:a|es)|unitat(?:s)?|quantitat|preu|import|descripci[oó]|concepte|treballs|total|iva|base)\b/i.test(result);
  }
  function practicalKeyword(job,budget){
    const candidates=[job?.keyword,budget?.keyword,budget?.workKeyword,job?.displayKeyword,job?.title,budget?.title];
    for(const candidate of candidates){
      const value=cleanKeyword(candidate);
      if(value && !genericKeyword(value) && !/^(?:factura|certificaci[oó]|materials?\s+i\s+m\.?o\.?)$/i.test(value)) return value;
    }
    const raw=text(job?.title||budget?.title||'');
    const words=raw.split(/\s+/).filter(Boolean);
    const useful=words.filter(word=>!/^\d+[\w/-]*$/.test(word)).slice(0,7).join(' ');
    return cleanKeyword(useful||job?.workAddress||job?.address||'') || 'Obra pendent de revisar';
  }
  function workAddress(job,budget){
    const candidates=[job?.workAddress,job?.address,budget?.workAddress,budget?.address];
    return candidates.map(text).find(value=>value&&!badWorkAddress(value))||'';
  }
  function workCity(job,budget){ return text(job?.workCity||job?.city||budget?.workCity||budget?.city||''); }
  function workPostal(job,budget){ return text(job?.workPostalCode||job?.postalCode||budget?.workPostalCode||budget?.postalCode||''); }
  function returnToJob(jobId,tab='resum'){
    const id=jobId||currentReturnJob();
    state.v09139ReturnJobId='';
    state.v09139ReturnJobTab='';
    if(id) state.selectedJobId=id;
    state.view='obres';
    if(tab) state.obresJobTab=tab;
    closeModal();
    try{ teimor09136RenderObres(false); }catch(error){ console.error(error); render(); }
  }
  function ensureShape(){
    if(typeof teimor0913EnsureData==='function') teimor0913EnsureData();
    data.clients=safeArray(data.clients);
    data.jobs=safeArray(data.jobs);
    data.budgets=safeArray(data.budgets);
    data.invoices=safeArray(data.invoices);
    data.certifications=safeArray(data.certifications);
    data.library=safeArray(data.library);
    data.hores=safeArray(data.hores);
    data.materials=safeArray(data.materials);
    data.agenda=safeArray(data.agenda);
    data.attachments=safeArray(data.attachments);
    data.settings=data.settings||{};
    if(!num(data.settings.officialHourRate)) data.settings.officialHourRate=31;
    if(!num(data.settings.laborerHourRate)) data.settings.laborerHourRate=27;
    state.v09139ReturnJobId=state.v09139ReturnJobId||'';
    state.v09139ReturnJobTab=state.v09139ReturnJobTab||'';
    state.v09139ImportContext=state.v09139ImportContext||null;
  }
  ensureShape();
  data.meta=data.meta||{};
  data.meta.version=VERSION;

  /* La identificació visible usa una frase pràctica, però no destrueix el
     títol llarg ni les dades importades. */
  const baseBuildModelsV09139=teimor09136BuildModels;
  teimor09136BuildModels=function(){
    const built=baseBuildModelsV09139();
    built.models.forEach(model=>{
      model.keyword=practicalKeyword(model.job,model.budgets[0]);
      model.address=workAddress(model.job,model.budgets[0])||model.address||'';
      model.city=workCity(model.job,model.budgets[0])||model.city||'';
    });
    return built;
  };

  function clientOptions(selected){
    return '<option value="">Client pendent</option>'+options(data.clients,selected,c=>c.name||c.id);
  }
  function jobEditorHtml(job){
    const address=workAddress(job,null);
    const city=workCity(job,null);
    const postal=workPostal(job,null);
    const keyword=practicalKeyword(job,null);
    const rateOfficial=num(job.officialRate)||num(data.settings.officialHourRate)||31;
    const rateLaborer=num(job.laborerRate)||num(data.settings.laborerHourRate)||27;
    return '<div class="toolbar"><div><h2>Editar fitxa d’obra</h2><p class="muted">Aquesta edició queda en primer pla i no modifica la pantalla que hi ha a sota fins que guardis.</p></div></div>'+
      '<form id="v09139JobForm" class="form-grid" data-job-id="'+esc(job.id)+'">'+
      '<label class="wide">Identificació pràctica de l’obra<input name="keyword" required value="'+esc(keyword==='Obra pendent de revisar'?'':keyword)+'" placeholder="Ex.: Coberta nau · Impermeabilització CP edifici"></label>'+
      '<label class="wide">Client<select name="clientId">'+clientOptions(job.clientId)+'</select></label>'+
      '<label>Any<input name="year" type="number" min="2000" max="2100" value="'+esc(job.year||new Date().getFullYear())+'"></label>'+ 
      '<label>Estat<select name="status"><option '+(job.status==='Activa'?'selected':'')+'>Activa</option><option '+(job.status==='En curs'?'selected':'')+'>En curs</option><option '+(job.status==='Pendent'?'selected':'')+'>Pendent</option><option '+(job.status==='Acabada'?'selected':'')+'>Acabada</option><option '+(job.status==='Històrica'?'selected':'')+'>Històrica</option></select></label>'+ 
      '<label class="wide">Descripció completa / títol intern<input name="title" value="'+esc(job.title||keyword)+'" placeholder="Descripció llarga si cal"></label>'+ 
      '<label class="wide">Adreça de l’obra<input name="workAddress" value="'+esc(address)+'" placeholder="Carrer, número, local o nau"></label>'+ 
      '<label>Població obra<input name="workCity" value="'+esc(city)+'"></label>'+ 
      '<label>Codi postal obra<input name="workPostalCode" value="'+esc(postal)+'"></label>'+ 
      '<label>Preu hora oficial<input name="officialRate" type="number" min="0" step="0.01" value="'+esc(rateOfficial)+'"></label>'+ 
      '<label>Preu hora manobre / ajudant<input name="laborerRate" type="number" min="0" step="0.01" value="'+esc(rateLaborer)+'"></label>'+ 
      '<label class="full">Notes<textarea name="notes">'+esc(job.notes||'')+'</textarea></label>'+ 
      '<div class="actions full"><button class="primary" type="submit">Guardar obra</button><button class="ghost" type="button" data-v09139-cancel-job>Cancel·lar</button></div></form>';
  }
  function saveJobFromFicha(event){
    event.preventDefault();
    const form=event.target;
    const fields=formObj(form);
    const id=form.dataset.jobId||fields.id;
    const old=byId(data.jobs,id);
    if(!old) return alert('No s’ha trobat aquesta obra.');
    const keyword=cleanKeyword(fields.keyword)||cleanKeyword(fields.title)||'Obra pendent de revisar';
    const title=text(fields.title)||keyword;
    const address=text(fields.workAddress);
    const city=text(fields.workCity);
    const postal=text(fields.workPostalCode);
    const updated={...old,id,year:Number(fields.year)||new Date().getFullYear(),clientId:fields.clientId||'',keyword,title,displayKeyword:keyword,address,workAddress:address,city,workCity:city,postalCode:postal,workPostalCode:postal,status:fields.status||old.status||'Activa',officialRate:num(fields.officialRate),laborerRate:num(fields.laborerRate),notes:fields.notes||''};
    const index=data.jobs.findIndex(item=>item.id===id);
    if(index<0) return alert('No s’ha trobat l’obra per guardar.');
    data.jobs[index]=updated;
    data.budgets.filter(item=>item.jobId===id).forEach(budget=>{
      if(!text(budget.keyword)||genericKeyword(budget.keyword)) budget.keyword=keyword;
      if(!text(budget.workAddress)||badWorkAddress(budget.workAddress)) budget.workAddress=address;
      if(!text(budget.workCity)) budget.workCity=city;
      if(!text(budget.workPostalCode)) budget.workPostalCode=postal;
    });
    saveData();
    const tab=state.obresJobTab||'resum';
    closeModal();
    state.view='obres'; state.selectedJobId=id;
    try{ teimor09136RenderObres(false); }catch(error){ console.error(error); render(); }
    if(tab) state.obresJobTab=tab;
  }
  function openJobEditor(jobId){
    ...
    openModal(jobEditorHtml(job));
    const form=document.getElementById('v09139JobForm');
    if(form) form.onsubmit=saveJobFromFicha;
    document.querySelectorAll('[data-v09139-cancel-job]').forEach(button=>button.onclick=()=>closeModal());
    form?.querySelector('[name="keyword"]')?.focus();
  }
  teimor09135OpenJobEditor=openJobEditor;

  /* Pressupostos oberts des de la fitxa tornen a la mateixa pestanya en
     guardar o cancel·lar. */
  const baseOpenBudgetV09139=openBudgetModal;
  openBudgetModal=function(id=''){
    if(hasOverlay() && state.selectedJobId && !state.v09139ReturnJobId){
      state.v09139ReturnJobId=state.selectedJobId;
      state.v09139ReturnJobTab='pressupost';
    }
    const result=baseOpenBudgetV09139(id);
    const returnJob=state.v09139ReturnJobId;
    if(returnJob){
      const budgetForm=document.getElementById('budgetForm');
      const job=byId(data.jobs,returnJob);
      if(budgetForm && job){
        const jobField=budgetForm.querySelector('[name="jobId"]');
        const clientField=budgetForm.querySelector('[name="clientId"]');
        if(jobField && !jobField.value) jobField.value=job.id;
        if(clientField && !clientField.value) clientField.value=job.clientId||'';
      }
      document.querySelectorAll('[data-render-budgets]').forEach(button=>button.onclick=()=>returnToJob(returnJob,'pressupost'));
    }
    return result;
  };
  const baseSaveBudgetV09139=saveBudget;
  saveBudget=function(event){
    const returnJob=state.v09139ReturnJobId;
    if(!returnJob) return baseSaveBudgetV09139(event);
    const tab=state.v09139ReturnJobTab||'pressupost';
    baseSaveBudgetV09139(event);
    returnToJob(returnJob,tab);
  };

  function budgetForJob(jobId,budgetId){ return byId(data.budgets,budgetId)||data.budgets.find(item=>item.jobId===jobId)||null; }
  function invoiceLineRows(budget,doc){
    const existing=safeArray(doc?.lines||doc?.invoiceLines);
    if(existing.length) return existing.map(line=>({...line,id:line.id||uid('IL')}));
    const budgetLines=safeArray(budget?.lines);
    if(budgetLines.length){
      return budgetLines.map(source=>{
        const billed=(data.invoices||[]).filter(item=>item.budgetId===budget.id&&item.id!==doc?.id).flatMap(item=>safeArray(item.lines||item.invoiceLines)).filter(line=>line.budgetLineId===source.id).reduce((sum,line)=>sum+num(line.qty),0);
        const budgetQty=num(source.qty);
        const remaining=budgetQty>0?Math.max(0,budgetQty-billed):1;
        return {id:uid('IL'),budgetLineId:source.id,code:source.code||'',chapter:source.chapter||'',unit:source.unit||'',concept:source.concept||'',longDesc:source.longDesc||'',qty:remaining,unitPrice:num(source.unitPrice),total:remaining*num(source.unitPrice)};
      });
    }
    if(budget) return [{id:uid('IL'),budgetLineId:'',unit:'',concept:budget.title||'Treballs de l’obra',longDesc:'',qty:1,unitPrice:budgetBase(budget),total:budgetBase(budget)}];
    return [{id:uid('IL'),unit:'',concept:'Nova feina / administració',longDesc:'',qty:1,unitPrice:0,total:0}];
  }
  function invoiceLineRowHtml(line){
    return '<tr data-v09139-invoice-line="1" data-line-id="'+esc(line.id)+'"><td><input data-v09139-line-field="concept" value="'+esc(line.concept||'')+'" placeholder="Feina / administració"></td><td><input data-v09139-line-field="longDesc" value="'+esc(line.longDesc||'')+'" placeholder="Descripció"></td><td><input data-v09139-line-field="unit" value="'+esc(line.unit||'')+'" placeholder="ud"></td><td><input class="num" type="number" step="0.0001" data-v09139-line-field="qty" value="'+esc(line.qty??0)+'"></td><td><input class="num" type="number" step="0.01" data-v09139-line-field="unitPrice" value="'+esc(line.unitPrice??0)+'"></td><td class="num"><strong data-v09139-line-total>'+money(num(line.qty)*num(line.unitPrice))+'</strong></td><td><button type="button" class="danger small" data-v09139-remove-line>Eliminar</button></td></tr>';
  }
  function nextFinancialNumber(kind,date){
    const year=(parseDateValue(date)||String(date||today())).slice(0,4)||String(new Date().getFullYear());
    const prefix=kind==='certificacio'?'CERT':'FAC';
    const docs=kind==='certificacio'?data.certifications:data.invoices;
    const max=docs.reduce((highest,item)=>{
      const value=String(item.number||'');
      const match=value.match(new RegExp('(?:'+prefix+'[- ]?)?'+year+'[-/](\\d+)$','i'));
      return match?Math.max(highest,Number(match[1])||0):highest;
    },0);
    return prefix+'-'+year+'-'+String(max+1).padStart(3,'0');
  }
  function financialStateOptions(kind,doc){
    if(kind==='certificacio') return '<option value="Pendent" '+(!doc?.status||/pendent/i.test(doc.status)?'selected':'')+'>Pendent</option><option value="Emesa" '+(/emesa|accept/i.test(doc?.status||'')?'selected':'')+'>Emesa</option><option value="Pagada" '+(/pagad|cobrat/i.test(doc?.status||'')?'selected':'')+'>Pagada</option>';
    const proforma=doc?.proforma||/proforma/i.test(doc?.status||'');
    const definitive=!proforma&&/definitiva|factura/i.test(doc?.status||'');
    const imported=!!doc&&!proforma&&!definitive&&!/pagad|cobrat|anul/i.test(doc?.status||'');
    return '<option value="Proforma" '+(proforma||!doc?'selected':'')+'>Proforma pendent</option><option value="Definitiva" '+(definitive?'selected':'')+'>Factura definitiva (Verifactu)</option><option value="Importada" '+(imported?'selected':'')+'>Importada / pendent de revisar</option><option value="Pagada" '+(/pagad|cobrat/i.test(doc?.status||'')?'selected':'')+'>Pagada</option><option value="Anul·lada" '+(/anul/i.test(doc?.status||'')?'selected':'')+'>Anul·lada</option>';
  }
  function financialFormHtml(doc,kind,jobId,budgetId){
    const job=byId(data.jobs,jobId)||currentJob(jobId)||{};
    const budget=budgetForJob(jobId,budgetId||doc?.budgetId);
    const lines=invoiceLineRows(budget,doc);
    const isNew=!doc?.id;
    const id=doc?.id||uid(kind==='certificacio'?'CERT':'FAC');
    const selectedJob=doc?.jobId||budget?.jobId||job.id||'';
    const selectedClient=doc?.clientId||budget?.clientId||job.clientId||'';
    const selectedBudget=doc?.budgetId||budget?.id||'';
    const defaultNumber=doc?.number||nextFinancialNumber(kind,doc?.date||today());
    const concept=doc?.concept||budget?.title||practicalKeyword(job,budget)||'';
    const base=doc?.base||(!doc?.id?lines.reduce((sum,line)=>sum+num(line.qty)*num(line.unitPrice),0):0);
    return '<div class="toolbar"><div><h2>'+ (isNew?(kind==='certificacio'?'Nova certificació':'Nova factura des del pressupost'):(kind==='certificacio'?'Editar certificació':'Editar factura'))+'</h2><p class="muted">'+(kind==='certificacio'?'La certificació queda relacionada amb l’obra i el pressupost.':'Les factures noves es guarden inicialment com a proforma; la factura definitiva/Verifactu es farà quan correspongui.')+'</p></div></div>'+ 
      '<form id="v09139FinancialForm" class="form-grid" data-kind="'+esc(kind)+'" data-edit-id="'+esc(doc?.id||'')+'" data-doc-id="'+esc(id)+'"><input type="hidden" name="editId" value="'+esc(doc?.id||'')+'"><input type="hidden" name="docId" value="'+esc(id)+'">'+
      '<label>Número / referència<input name="number" value="'+esc(defaultNumber)+'" required></label>'+ 
      '<label>Data<input name="date" type="date" value="'+esc(doc?.date||today())+'" required></label>'+ 
      '<label class="wide">Client<select name="clientId">'+clientOptions(selectedClient)+'</select></label>'+ 
      '<label class="wide">Obra<select name="jobId"><option value="">Sense obra</option>'+options(data.jobs,selectedJob,j=>`${j.year||''} · ${practicalKeyword(j,null)}`)+'</select></label>'+ 
      '<label class="wide">Pressupost<select name="budgetId"><option value="">Sense pressupost</option>'+options(data.budgets,selectedBudget,b=>`${b.number||b.id} · ${b.title||''}`)+'</select></label>'+ 
      '<label class="wide">Concepte<input name="concept" value="'+esc(concept)+'" placeholder="Feines, administració, ampliació..."></label>'+ 
      '<label>Estat<select name="documentState">'+financialStateOptions(kind,doc)+'</select></label>'+ 
      '<label>IVA %<input name="iva" type="number" step="0.01" value="'+esc(doc?.iva??data.settings.defaultIVA)+'"></label>'+ 
      (kind==='certificacio'?'<label>Percentatge certificat %<input name="percentage" type="number" step="0.01" value="'+esc(doc?.percentage||'')+'"></label>':'<label>Pagada<select name="paid"><option value="false" '+(!doc?.paid?'selected':'')+'>No</option><option value="true" '+(doc?.paid?'selected':'')+'>Sí</option></select></label>')+
      '<label class="full">Base manual s/IVA <input name="base" type="number" step="0.01" value="'+esc(base)+'" placeholder="Si no hi ha línies detallades"></label>'+ 
      '<div class="card full"><div class="toolbar"><div><h3>Feines a '+(kind==='certificacio'?'certificar':'facturar')+'</h3><p class="muted">Canvia les quantitats. Pots afegir una partida nova d’administració o una feina no prevista al pressupost.</p></div><button type="button" class="ghost" data-v09139-add-line>+ Afegir línia nova</button></div><div class="table-wrap"><table id="v09139InvoiceLines"><thead><tr><th>Concepte</th><th>Descripció</th><th>Ut.</th><th>Quant.</th><th>Preu/ut.</th><th>Total</th><th></th></tr></thead><tbody>'+lines.map(invoiceLineRowHtml).join('')+'</tbody></table></div><div class="budget-total">Base calculada: <strong id="v09139InvoiceBasePreview">'+money(base)+'</strong></div></div>'+ 
      '<label class="full">Notes<textarea name="notes">'+esc(doc?.notes||'')+'</textarea></label>'+ 
      '<div class="actions full"><button class="primary" type="submit">Guardar '+(kind==='certificacio'?'certificació':'factura')+'</button><button class="ghost" type="button" data-v09139-cancel-financial>Cancel·lar</button></div></form>';
  }
  function updateInvoiceLineTotals(){
    let base=0;
    document.querySelectorAll('#v09139InvoiceLines [data-v09139-invoice-line]').forEach(row=>{
      const qty=num(row.querySelector('[data-v09139-line-field="qty"]')?.value);
      const price=num(row.querySelector('[data-v09139-line-field="unitPrice"]')?.value);
      base+=qty*price;
      const total=row.querySelector('[data-v09139-line-total]'); if(total) total.textContent=money(qty*price);
    });
    const preview=document.getElementById('v09139InvoiceBasePreview');
    if(preview) preview.textContent=money(base);
  }
  function addInvoiceLine(){
    const tbody=document.querySelector('#v09139InvoiceLines tbody');
    if(!tbody) return;
    const line={id:uid('IL'),unit:'',concept:'Nova feina / administració',longDesc:'',qty:1,unitPrice:0};
    tbody.insertAdjacentHTML('beforeend',invoiceLineRowHtml(line));
    bindFinancialForm();
    const last=tbody.lastElementChild?.querySelector('[data-v09139-line-field="concept"]'); last?.focus();
  }
  function collectInvoiceLines(){
    return [...document.querySelectorAll('#v09139InvoiceLines [data-v09139-invoice-line]')].map(row=>({
      id:row.dataset.lineId||uid('IL'),
      concept:row.querySelector('[data-v09139-line-field="concept"]')?.value||'',
      longDesc:row.querySelector('[data-v09139-line-field="longDesc"]')?.value||'',
      unit:row.querySelector('[data-v09139-line-field="unit"]')?.value||'',
      qty:num(row.querySelector('[data-v09139-line-field="qty"]')?.value),
      unitPrice:num(row.querySelector('[data-v09139-line-field="unitPrice"]')?.value)
    })).map(line=>({...line,total:Number((line.qty*line.unitPrice).toFixed(2))})).filter(line=>line.concept||line.longDesc||line.qty||line.unitPrice);
  }
  function saveFinancialFromFicha(event){
    event.preventDefault();
    const form=event.target;
    const fields=formObj(form);
    const kind=form.dataset.kind==='certificacio'?'certificacio':'factura';
    const target=kind==='certificacio'?data.certifications:data.invoices;
    const old=byId(target,fields.editId)||{};
    const budget=byId(data.budgets,fields.budgetId);
    const job=byId(data.jobs,fields.jobId||budget?.jobId);
    const lines=collectInvoiceLines();
    const lineBase=lines.reduce((sum,line)=>sum+line.total,0);
    const base=lineBase>0?Number(lineBase.toFixed(2)):num(fields.base);
    const iva=num(fields.iva);
    const status=kind==='certificacio' ? (fields.documentState||'Pendent') : (fields.documentState==='Definitiva'?'Factura definitiva':fields.documentState==='Importada'?(old.status||'Importada · pendent de revisar'):fields.documentState==='Pagada'?'Pagada':fields.documentState==='Anul·lada'?'Anul·lada':'Proforma pendent');
    const item={...old,id:fields.docId||old.id||uid(kind==='certificacio'?'CERT':'FAC'),kind,documentType:kind==='certificacio'?'certificacio':'factura',number:text(fields.number)||nextFinancialNumber(kind,fields.date),date:fields.date||today(),clientId:fields.clientId||budget?.clientId||job?.clientId||'',jobId:budget?.jobId||fields.jobId||'',budgetId:fields.budgetId||'',type:old.type||'Client',concept:text(fields.concept)||'Feines de l’obra',base,iva,total:Number((base*(1+iva/100)).toFixed(2)),paid:fields.paid==='true',proforma:kind==='factura'&&status==='Proforma pendent',status,percentage:kind==='certificacio'?num(fields.percentage):old.percentage||0,notes:fields.notes||'',lines,invoiceLines:lines,source:old.source||'Manual des de fitxa'};
    if(item.budgetId && budget){ item.jobId=budget.jobId||item.jobId; item.clientId=budget.clientId||item.clientId; }
    item.matchStatus=item.jobId||item.budgetId?'manual':'pendent';
    item.matchConfidence=item.jobId||item.budgetId?100:0;
    item.matchReason=item.jobId||item.budgetId?'Relació creada/validada des de la fitxa d’obra.':'Sense obra ni pressupost vinculats.';
    const index=target.findIndex(existing=>existing.id===fields.editId||existing.id===item.id);
    if(index>=0) target[index]=item; else target.push(item);
    saveData();
    const returnJob=state.v09139ReturnJobId||item.jobId||state.selectedJobId;
    const returnTab=kind==='certificacio'?'certificacions':'factures';
    if(returnJob) returnToJob(returnJob,returnTab); else { closeModal(); state.view='invoices'; render(); }
  }
  function bindFinancialForm(){
    const form=document.getElementById('v09139FinancialForm');
    if(form) form.onsubmit=saveFinancialFromFicha;
    document.querySelectorAll('[data-v09139-cancel-financial]').forEach(button=>button.onclick=()=>{
      const id=state.v09139ReturnJobId;
      if(id) returnToJob(id,form?.dataset.kind==='certificacio'?'certificacions':'factures');
      else closeModal();
    });
    document.querySelectorAll('[data-v09139-add-line]').forEach(button=>button.onclick=addInvoiceLine);
    document.querySelectorAll('#v09139InvoiceLines input').forEach(input=>input.oninput=updateInvoiceLineTotals);
    document.querySelectorAll('[data-v09139-remove-line]').forEach(button=>button.onclick=()=>{ button.closest('tr')?.remove(); updateInvoiceLineTotals(); });
    updateInvoiceLineTotals();
  }
  function openFinancialEditor(id='',kind='factura',jobId='',budgetId=''){
    let doc='';
    if(id){ doc=kind==='certificacio'?byId(data.certifications,id):byId(data.invoices,id); if(!doc) return alert('No s’ha trobat el document.'); }
    const resolvedJob=jobId||doc?.jobId||(hasOverlay()?state.selectedJobId:'')||'';
    const resolvedBudget=budgetId||doc?.budgetId||'';
    if(resolvedJob){ state.v09139ReturnJobId=resolvedJob; state.v09139ReturnJobTab=kind==='certificacio'?'certificacions':'factures'; }
    openModal(financialFormHtml(doc,kind,resolvedJob,resolvedBudget));
    bindFinancialForm();
  }
  function openInvoiceFromBudget(button,kind){
    const budgetId=button.dataset.v09139BudgetId||'';
    const budget=byId(data.budgets,budgetId);
    if(!budget) return alert('No s’ha trobat el pressupost.');
    state.v09139ReturnJobId=budget.jobId||state.selectedJobId||'';
    state.v09139ReturnJobTab=kind==='certificacio'?'certificacions':'factures';
    openFinancialEditor('',kind,budget.jobId||'',budget.id);
  }
  teimor09134OpenInvoiceEditor=function(id){ openFinancialEditor(id,'factura'); };

  teimor09136BudgetsTab=function(model){
    const actions='<div class="actions"><button class="primary small" data-v09139-new-budget-job="'+esc(model.job.id)+'">+ Afegir pressupost</button></div>';
    if(!model.budgets.length) return '<div class="card"><div class="toolbar"><h3>Pressupostos</h3>'+actions+'</div>'+empty('Aquesta obra encara no té cap pressupost vinculat.')+'</div>';
    return '<div class="card"><div class="toolbar"><div><h3>Pressupostos vinculats</h3><p class="muted">Des d’aquí pots editar el pressupost, facturar una part i certificar-ne una altra.</p></div>'+actions+'</div>'+table(['Any','Número','Data','Concepte','Estat','Base','Total','Acció'],model.budgets.map(budget=>'<tr><td>'+esc(budgetYear(budget)||model.year||'')+'</td><td><strong>'+esc(budget.number||budget.id)+'</strong></td><td>'+dateDisplay(budget.date)+'</td><td>'+esc(budget.title||budget.keyword||'')+'</td><td>'+statusPill(budget.status||'')+'</td><td class="num">'+money(budgetBase(budget))+'</td><td class="num">'+money(budgetTotal(budget))+'</td><td class="nowrap"><button class="ghost small" data-open-budget="'+esc(budget.id)+'">Obrir / editar</button> <button class="primary small" data-v09139-create-invoice data-v09139-budget-id="'+esc(budget.id)+'">Factura parcial</button> <button class="ghost small" data-v09139-create-cert data-v09139-budget-id="'+esc(budget.id)+'">Certificació</button></td></tr>'))+'</div>';
  };
  teimor09136InvoicesTab=function(model){
    const newBudget=model.budgets[0];
    const top='<div class="actions"><button class="primary small" data-v09139-new-invoice data-v09139-job-id="'+esc(model.job.id)+'"'+(newBudget?' data-v09139-budget-id="'+esc(newBudget.id)+'"':'')+'>+ Crear factura'+(newBudget?' des del pressupost':'')+'</button><button class="ghost small" data-v09139-import-invoice data-v09139-job-id="'+esc(model.job.id)+'">Importar factura existent</button></div>';
    if(!model.invoices.length) return '<div class="card"><div class="toolbar"><div><h3>Factures</h3><p class="muted">Crea una factura parcial des del pressupost o importa una factura antiga i valida la relació.</p></div>'+top+'</div>'+empty('Aquesta obra encara no té cap factura vinculada.')+'</div>';
    const rows=model.invoices.map(invoice=>'<tr><td>'+esc(dateDisplay(invoice.date))+'</td><td><strong>'+esc(invoice.number||invoice.id)+'</strong></td><td>'+esc(invoice.concept||'')+'</td><td><select data-v09136-invoice-job="'+esc(invoice.id)+'">'+teimor09136InvoiceJobOptions(model,invoice)+'</select></td><td><select data-v09136-invoice-budget="'+esc(invoice.id)+'">'+teimor09136InvoiceBudgetOptions(model,invoice)+'</select></td><td class="num">'+money(invoiceBase(invoice))+'</td><td class="num">'+money(invoiceTotal(invoice))+'</td><td class="nowrap"><button class="primary small" data-v09136-save-invoice="'+esc(invoice.id)+'">Guardar relació</button> <button class="ghost small" data-edit-invoice="'+esc(invoice.id)+'">Editar</button> <button class="ghost small" data-v09134-invoice-pdf="'+esc(invoice.id)+'">Veure PDF</button> <button class="ghost small" data-v09132-download-invoice="'+esc(invoice.id)+'">Descarregar factura</button></td></tr>');
    return '<div class="card"><div class="toolbar"><div><h3>Factures vinculades</h3><p class="muted">La fitxa és el lloc de treball: editar, relacionar, previsualitzar i descarregar.</p></div>'+top+'</div>'+table(['Data','Número','Concepte','Obra','Pressupost','Base','Total','Acció'],rows)+'</div>';
  };
  teimor09136CertificationsTab=function(model){
    const budget=model.budgets[0];
    const top='<div class="actions"><button class="primary small" data-v09139-create-cert'+(budget?' data-v09139-budget-id="'+esc(budget.id)+'"':'')+'>+ Crear certificació parcial</button></div>';
    const body=model.certifications.length?teimor0913TraceCertificationHtml(model.certifications):empty('Aquesta obra encara no té certificacions vinculades.');
    return '<div class="card"><div class="toolbar"><div><h3>Certificacions</h3><p class="muted">Pots certificar només les quantitats que vulguis del pressupost.</p></div>'+top+'</div>'+body+(model.certifications.length?'<div class="table-wrap" style="margin-top:12px"><table><tbody>'+model.certifications.map(item=>'<tr><td>'+esc(item.number||item.id)+'</td><td><button class="ghost small" data-v09139-edit-cert="'+esc(item.id)+'">Editar</button></td></tr>').join('')+'</tbody></table></div>':'')+'</div>';
  };

  /* La fitxa no redirigeix a Arxius: guarda el fitxer i el relaciona amb
     l’obra. En un albarà, la detecció inicial del material surt del nom del
     fitxer i queda editable a Temps i materials. */
  function detectMaterialFromAttachment(meta){
    if(!/albar[aà]n|entrega|delivery/i.test(meta.category||meta.name||'')) return;
    const exists=data.materials.some(item=>item.attachmentId===meta.id);
    if(exists) return;
    const name=text(meta.name||'Albarà').replace(/\.[^.]+$/,'').replace(/[_-]+/g,' ');
    data.materials.push({id:uid('MAT'),jobId:meta.jobId||'',attachmentId:meta.id,name,concept:name,unit:'ud',qty:1,unitPrice:0,total:0,source:'Albarà detectat',status:'Pendent de completar',notes:'Detectat pel nom de l’albarà; revisa material, quantitat i preu.'});
  }
  async function saveJobAttachment(event){
    event.preventDefault();
    const form=event.target;
    ...
    const fields=formObj(form);
    const id=uid('ARX');
    const include=fields.includeInJson==='true';
    const meta={id,name:file.name,type:file.type,size:file.size,category:fields.category||'Albarà',clientId:fields.clientId||byId(data.jobs,fields.jobId)?.clientId||'',jobId:fields.jobId||'',budgetId:fields.budgetId||'',includeInJson:include,notes:fields.notes||'',createdAt:new Date().toISOString()};
    const dataUrl=include?await fileToDataUrl(file):'';
    await idbPut({id,blob:file,dataUrl,meta});
    data.attachments.push(meta); detectMaterialFromAttachment(meta); saveData();
    returnToJob(meta.jobId,form.dataset.onlyAlbarans==='1'?'albarans':'documentacio');
  }
  async function deleteJobAttachment(id,jobId,tab){
    if(!confirm('Eliminar aquest arxiu de l’obra?')) return;
    data.attachments=data.attachments.filter(item=>item.id!==id);
    data.materials=data.materials.filter(item=>item.attachmentId!==id);
    try{ await idbDelete(id); }catch(error){ console.warn(error); }
    saveData(); returnToJob(jobId,tab);
  }
  function fichaAttachmentForm(model,onlyAlbarans){
    const category=onlyAlbarans?'Albarà':'Albarà';
    return '<div class="card-tight notice-blue" style="margin-bottom:12px">'+(onlyAlbarans?'Adjunta l’albarà aquí. La fitxa el relaciona amb l’obra i crea una línia de material pendent de revisar.':'Pots adjuntar documentació de l’obra sense sortir de la fitxa.')+'</div><form id="v09139AttachmentForm" class="form-grid" data-job-id="'+esc(model.job.id)+'" data-only-albarans="'+(onlyAlbarans?'1':'0')+'"><input type="hidden" name="jobId" value="'+esc(model.job.id)+'"><input type="hidden" name="clientId" value="'+esc(model.job.clientId||'')+'">'+(onlyAlbarans?'<input type="hidden" name="category" value="Albarà">':'<label>Categoria<select name="category"><option>Albarà</option><option>Factura</option><option>Pressupost original</option><option>Foto</option><option>Altres</option></select></label>')+'<label class="wide">Arxiu<input name="file" type="file" required></label><label>Incloure al JSON<select name="includeInJson"><option value="false">No, només referència local</option><option value="true">Sí, còpia transferible</option></select></label><label class="wide">Pressupost relacionat<select name="budgetId"><option value="">Sense pressupost</option>'+options(model.budgets,'',b=>`${b.number||b.id} · ${b.title||''}`)+'</select></label><label class="full">Notes / material que hi veus<textarea name="notes" placeholder="Ex.: rajola 60x60, sacs de morter..."></textarea></label><div class="actions full"><button class="primary">Guardar arxiu a l’obra</button></div></form>';
  }
  teimor09136DocumentsTab=function(model,onlyAlbarans=false){
    const rows=(model.attachments||[]).filter(item=>!onlyAlbarans||/albar[aà]n|entrega|delivery/i.test([item.name,item.category,item.type,item.notes].filter(Boolean).join(' ')));
    const list=rows.length?table(['Nom','Categoria','Mida','Notes','Acció'],rows.map(item=>'<tr><td><strong>'+esc(item.name||item.id)+'</strong></td><td>'+esc(item.category||'')+'</td><td class="num">'+(num(item.size)/1024).toFixed(1)+' KB</td><td>'+esc(item.notes||'')+'</td><td class="nowrap"><button class="ghost small" data-download-attachment="'+esc(item.id)+'">Obrir</button> <button class="danger small" data-v09139-delete-attachment="'+esc(item.id)+'" data-v09139-job-id="'+esc(model.job.id)+'">Eliminar</button></td></tr>')):empty(onlyAlbarans?'No hi ha albarans vinculats a aquesta obra.':'No hi ha documentació vinculada a aquesta obra.');
    return '<div class="card"><div class="toolbar"><div><h3>'+(onlyAlbarans?'Albarans':'Documentació de l’obra')+'</h3><p class="muted">Gestiona els arxius des d’aquesta fitxa.</p></div></div>'+fichaAttachmentForm(model,onlyAlbarans)+'</div><div class="card"><h3>Arxius vinculats</h3>'+list+'</div>';
  };

  function saveHour(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const item={id:uid('HRS'),jobId:fields.jobId,date:fields.date||today(),officialHours:num(fields.officialHours),officialRate:num(fields.officialRate),laborerHours:num(fields.laborerHours),laborerRate:num(fields.laborerRate),hours:num(fields.officialHours)+num(fields.laborerHours),total:Number((num(fields.officialHours)*num(fields.officialRate)+num(fields.laborerHours)*num(fields.laborerRate)).toFixed(2)),concept:fields.concept||'Jornada d’obra',notes:fields.notes||'',status:'Manual'};
    data.hores.push(item); saveData(); returnToJob(item.jobId,'temps');
  }
  function saveMaterial(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const item={id:uid('MAT'),jobId:fields.jobId,date:fields.date||today(),concept:fields.concept||'Material',name:fields.concept||'Material',unit:fields.unit||'ud',qty:num(fields.qty),unitPrice:num(fields.unitPrice),total:Number((num(fields.qty)*num(fields.unitPrice)).toFixed(2)),source:fields.source||'Manual',notes:fields.notes||'',status:'Manual'};
    data.materials.push(item); saveData(); returnToJob(item.jobId,'temps');
  }
  function detectJobMaterials(jobId){
    data.attachments.filter(item=>item.jobId===jobId&&/albar[aà]n|entrega|delivery/i.test([item.name,item.category,item.notes].join(' '))).forEach(detectMaterialFromAttachment);
    saveData(); returnToJob(jobId,'temps');
  }
  async function deleteHour(id,jobId){ if(!confirm('Eliminar aquesta imputació d’hores?')) return; data.hores=data.hores.filter(item=>item.id!==id); saveData(); returnToJob(jobId,'temps'); }
  async function deleteMaterial(id,jobId){ if(!confirm('Eliminar aquest material?')) return; data.materials=data.materials.filter(item=>item.id!==id); saveData(); returnToJob(jobId,'temps'); }
  teimor09136TimeMaterialsTab=function(model){
    const officialRate=num(model.job.officialRate)||num(data.settings.officialHourRate)||31;
    const laborerRate=num(model.job.laborerRate)||num(data.settings.laborerHourRate)||27;
    const hours=model.hours||[];
    const materials=(model.materials||[]).filter(item=>item.attachmentId||item.source||item.name||item.concept).slice(0,200);
    const hourRows=hours.length?table(['Data','Concepte','Oficial h','Manobre h','Total','Acció'],hours.map(item=>'<tr><td>'+esc(dateDisplay(item.date))+'</td><td>'+esc(item.concept||item.title||item.notes||'')+'</td><td class="num">'+num(item.officialHours||0).toFixed(2)+' · '+money(item.officialRate||0)+'</td><td class="num">'+num(item.laborerHours||0).toFixed(2)+' · '+money(item.laborerRate||0)+'</td><td class="num">'+money(item.total||item.cost||0)+'</td><td><button class="danger small" data-v09139-delete-hour="'+esc(item.id)+'" data-v09139-job-id="'+esc(model.job.id)+'">Eliminar</button></td></tr>')):empty('Encara no hi ha jornades imputades.');
    const matRows=materials.length?table(['Origen','Material','Ut.','Quantitat','Preu/ut.','Total','Acció'],materials.map(item=>'<tr><td>'+esc(item.sourceBudget||item.source||item.attachmentId||'')+'</td><td>'+esc(item.concept||item.name||'')+'</td><td>'+esc(item.unit||'')+'</td><td class="num">'+num(item.qty||item.quantity||0).toFixed(3)+'</td><td class="num">'+money(item.unitPrice||item.price||0)+'</td><td class="num">'+money(item.total||lineTotal(item))+'</td><td>'+(item.id&&data.materials.some(candidate=>candidate.id===item.id)?'<button class="danger small" data-v09139-delete-material="'+esc(item.id)+'" data-v09139-job-id="'+esc(model.job.id)+'">Eliminar</button>':'')+'</td></tr>')):empty('Encara no hi ha materials separats.');
    return '<div class="grid two"><div class="card"><div class="toolbar"><div><h3>Imputació de temps</h3><p class="muted">Preus per defecte: oficial '+money(officialRate)+'/h · manobre '+money(laborerRate)+'/h. Els pots canviar per obra o a Configuració.</p></div></div><form id="v09139HourForm" class="form-grid" data-job-id="'+esc(model.job.id)+'"><input type="hidden" name="jobId" value="'+esc(model.job.id)+'"><label>Dia treballat<input name="date" type="date" value="'+today()+'" required></label><label class="wide">Concepte<input name="concept" value="Jornada d’obra"></label><label>Hores oficial<input name="officialHours" type="number" step="0.25" min="0" value="0"></label><label>Preu/h oficial<input name="officialRate" type="number" step="0.01" min="0" value="'+esc(officialRate)+'"></label><label>Hores manobre<input name="laborerHours" type="number" step="0.25" min="0" value="0"></label><label>Preu/h manobre<input name="laborerRate" type="number" step="0.01" min="0" value="'+esc(laborerRate)+'"></label><label class="full">Notes<textarea name="notes"></textarea></label><div class="actions full"><button class="primary">Afegir jornada</button></div></form>'+hourRows+'</div><div class="card"><div class="toolbar"><div><h3>Materials</h3><p class="muted">Els albarans creen una línia pendent a revisar; completa-la aquí.</p></div><button class="ghost small" type="button" data-v09139-detect-materials="'+esc(model.job.id)+'">Detectar albarans</button></div><form id="v09139MaterialForm" class="form-grid" data-job-id="'+esc(model.job.id)+'"><input type="hidden" name="jobId" value="'+esc(model.job.id)+'"><label>Dia<input name="date" type="date" value="'+today()+'" required></label><label class="wide">Material / concepte<input name="concept" required placeholder="Rajola, morter, làmina..."></label><label>Unitat<input name="unit" value="ud"></label><label>Quantitat<input name="qty" type="number" step="0.001" min="0" value="1"></label><label>Preu/ut. €<input name="unitPrice" type="number" step="0.01" min="0" value="0"></label><label class="wide">Origen<input name="source" placeholder="Albarà núm. / proveïdor"></label><label class="full">Notes<textarea name="notes"></textarea></label><div class="actions full"><button class="primary">Afegir material</button></div></form>'+matRows+'</div></div>';
  };

  /* Ajustos horaris visibles també a Configuració. */
  const baseRenderSettingsV09139=renderSettings;
  renderSettings=function(){
    baseRenderSettingsV09139();
    const content=document.getElementById('content');
    if(!content) return;
    content.insertAdjacentHTML('beforeend','<div class="card"><h2>Preus hora per defecte</h2><p class="muted">S’utilitzen en les noves imputacions de temps. A cada obra es poden substituir.</p><form id="v09139RatesForm" class="form-grid"><label>Oficial €/h<input name="officialHourRate" type="number" step="0.01" min="0" value="'+esc(data.settings.officialHourRate||31)+'"></label><label>Manobre / ajudant €/h<input name="laborerHourRate" type="number" step="0.01" min="0" value="'+esc(data.settings.laborerHourRate||27)+'"></label><div class="actions full"><button class="primary">Guardar preus hora</button></div></form></div>');
    const form=document.getElementById('v09139RatesForm');
    if(form) form.onsubmit=event=>{ event.preventDefault(); const fields=formObj(form); data.settings.officialHourRate=num(fields.officialHourRate); data.settings.laborerHourRate=num(fields.laborerHourRate); saveData(); alert('Preus hora guardats.'); renderSettings(); };
  };

  /* Importació financera des de la fitxa: reutilitza el lector existent, però
     el preview i la confirmació continuen dins del modal de l’obra. */
  function openInvoiceImport(jobId){
    state.financialDraft=null;
    state.v09139ImportContext={jobId,kind:'factura',prepared:false};
    openModal('<h2>Importar factura existent a aquesta obra</h2><p class="muted">Selecciona l’Excel/CSV o ZIP. Després podràs validar el client, obra i pressupost abans de confirmar.</p><div class="filter-grid"><label>Tipus<select id="v0913FinancialKind"><option value="factura" selected>Factura</option><option value="certificacio">Certificació</option></select></label></div><label class="primary file-label" style="display:inline-block">Seleccionar factures<input id="v09139FichaInvoiceInput" type="file" multiple accept=".xls,.xlsx,.xlsm,.csv,.zip,.rar" hidden></label><div id="v09139FichaImportStatus" class="small-text" style="margin-top:12px">Encara no s’ha llegit cap fitxer.</div>');
    const input=document.getElementById('v09139FichaInvoiceInput');
    if(input) input.onchange=event=>handleFinancialImportV0913([...event.target.files]);
  }
  const baseHandleFinancialV09139=handleFinancialImportV0913;
  handleFinancialImportV0913=async function(files){
    await baseHandleFinancialV09139(files);
    const draft=state.financialDraft;
    if(!draft) return;
    const direct=new Map((files||[]).map(file=>[file.webkitRelativePath||file.name,file]));
    draft.documents.forEach(doc=>{
      const file=direct.get(doc.sourceFile)||direct.get(teimor0913SourceName(doc.sourceFile));
      if(file && typeof teimor09132RememberSource==='function') teimor09132RememberSource(doc,{name:file.name,blob:file});
    });
    if(state.v09139ImportContext) renderImporter();
  };
  const baseRenderImporterV09139=renderImporter;
  renderImporter=function(){
    const context=state.v09139ImportContext;
    if(context && state.financialDraft){
      const draft=state.financialDraft;
      if(!context.prepared){
        const job=byId(data.jobs,context.jobId);
        draft.documents.forEach(doc=>{
          if(!doc.jobId) doc.jobId=context.jobId;
          if(job && !doc.clientId) doc.clientId=job.clientId||'';
          if(!doc.jobId) return;
          doc.matchStatus=doc.matchStatus==='automatica'?'automatica':'manual';
          doc.matchReason=doc.matchReason||'Importada des de la fitxa de l’obra; validar abans de confirmar.';
        });
        context.prepared=true;
      }
      let preview=teimor0913FinancialPreviewHtml(draft);
      preview=preview.replace('<button class="ghost" data-go="obres">Veure obres</button>','<button class="ghost" data-v09139-cancel-import>Cancel·lar</button>');
      openModal('<div class="notice-blue card-tight"><strong>Importació dins de la fitxa:</strong> revisa les coincidències i confirma només quan el número, client i pressupost siguin correctes.</div>'+preview);
      document.querySelectorAll('[data-v0913-financial-field]').forEach(field=>field.onchange=updateFinancialDraftFieldV0913);
      const confirm=document.getElementById('v0913ConfirmFinancialImport'); if(confirm) confirm.onclick=confirmFinancialImportV0913;
      const cancel=document.getElementById('v0913DiscardFinancialImport'); if(cancel) cancel.onclick=discardFinancialImportV0913;
      document.querySelectorAll('[data-v09139-cancel-import]').forEach(button=>button.onclick=()=>discardFinancialImportV0913());
      return;
    }
    return baseRenderImporterV09139();
  };
  const baseConfirmFinancialV09139=confirmFinancialImportV0913;
  confirmFinancialImportV0913=async function(){
    const context=state.v09139ImportContext;
    await baseConfirmFinancialV09139();
    if(context){
      state.v09139ImportContext=null;
      returnToJob(context.jobId,'factures');
    }
  };
  const baseDiscardFinancialV09139=discardFinancialImportV0913;
  discardFinancialImportV0913=function(){
    const context=state.v09139ImportContext;
    if(context){ state.financialDraft=null; state.v09139ImportContext=null; returnToJob(context.jobId,'factures'); return; }
    return baseDiscardFinancialV09139();
  };

  /* Impressió comercial: mostra les línies de factura i només les
     referències útils d’obra/pressupost, no missatges interns de relació. */
  openInvoicePreview=function(id){
    const invoice=byId(data.invoices,id);
    if(!invoice) return alert('No s’ha trobat aquesta factura.');
    const budget=teimor09135InvoiceBudget(invoice)||{};
    ...
    const client=byId(data.clients,invoice.clientId)||{};
    const snapshot=invoice.clientSnapshot||{};
    const settings=data.settings.contractista||{};
    const name=client.name||snapshot.name||'Client pendent de revisar';
    const nif=client.nif||snapshot.nif||'';
    const fiscal=[client.fiscalAddress||snapshot.fiscalAddress,client.postalCode||snapshot.postalCode,client.city||snapshot.city].filter(Boolean).join(' · ');
    const keyword=practicalKeyword(job,budget);
    const address=workAddress(job,budget);
    const city=workCity(job,budget);
    const lines=safeArray(invoice.lines||invoice.invoiceLines);
    const rows=lines.length?lines.map(line=>'<tr><td><strong>'+esc(line.concept||'')+'</strong>'+(line.longDesc?'<div class="preview-desc">'+esc(line.longDesc)+'</div>':'')+'</td><td class="num">'+esc(line.qty??'')+'</td><td>'+esc(line.unit||'')+'</td><td class="num">'+money(line.unitPrice)+'</td><td class="num">'+money(line.total||num(line.qty)*num(line.unitPrice))+'</td></tr>').join(''):'<tr><td><strong>'+esc(invoice.concept||'Factura')+'</strong></td><td class="num">1</td><td>ud</td><td class="num">'+money(invoiceBase(invoice))+'</td><td class="num">'+money(invoiceBase(invoice))+'</td></tr>';
    const html='<div class="preview-toolbar actions"><button class="primary" onclick="window.print()">Imprimir / guardar PDF</button><button class="ghost" onclick="window.close()">Tancar</button></div><div class="a4-sheet"><div class="preview-colorbar"></div><div class="preview-header"><div><h1>'+esc(settings.name||'TEIMOR')+'</h1><p>'+esc(settings.nif||'')+'<br>'+esc(settings.address||'')+'<br>'+esc(settings.city||'')+'<br>'+esc(settings.phone||'')+'</p></div><div class="client-box"><span>Client</span><strong>'+esc(name)+'</strong><br>'+esc(nif)+'<br>'+esc(fiscal)+'</div></div><div class="preview-meta"><div><strong>Data:</strong> '+dateDisplay(invoice.date)+'</div><div><strong>Factura:</strong> '+esc(invoice.number||invoice.id)+'</div><div><strong>Estat:</strong> '+esc(invoice.status||'Proforma pendent')+'</div></div><div style="border:1px solid #fed7aa;border-left:5px solid #c2410c;background:#fff7ed;padding:8px 10px;margin:0 0 8mm;font-size:12px"><div><strong>Referència d’obra:</strong> '+esc(keyword)+'</div>'+(address||city?'<div><strong>Adreça de l’obra:</strong> '+esc([address,city].filter(Boolean).join(' · '))+'</div>':'')+(budget.number?'<div><strong>Pressupost:</strong> '+esc(budget.number)+'</div>':'')+'</div><h2>'+esc(invoice.concept||keyword||'Factura')+'</h2><table class="preview-table"><thead><tr><th>Concepte / descripció</th><th>Quant.</th><th>Ut.</th><th>Preu/ut.</th><th>Total</th></tr></thead><tbody>'+rows+'</tbody></table><div class="preview-totals"><div>Base s/IVA: <strong>'+money(invoiceBase(invoice))+'</strong></div><div>IVA '+num(invoice.iva).toFixed(2)+'%: <strong>'+money(invoiceTotal(invoice)-invoiceBase(invoice))+'</strong></div><div>Total: <strong>'+money(invoiceTotal(invoice))+'</strong></div></div>'+(teimor09135PrintableNotes(invoice)?'<div class="preview-notes">'+esc(teimor09135PrintableNotes(invoice))+'</div>':'')+'</div>';
    const w=window.open('', '_blank');
    if(!w) return alert('El navegador ha bloquejat la finestra de previsualització. Permet finestres emergents per aquesta app.');
    w.document.write('<!doctype html><html lang="ca"><head><meta charset="utf-8"><title>Factura '+esc(invoice.number||invoice.id)+'</title><style>'+previewCss()+'</style></head><body>'+html+'</body></html>');
    w.document.close();
  };

  /* Accions que es perden quan la fitxa passa al modal: es tornen a lligar
     sempre després de cada render. */
  const baseRenderObresV09139=teimor09136RenderObres;
  teimor09136RenderObres=function(prepareData){
    const result=baseRenderObresV09139(prepareData);
    const scope=document.getElementById('v09137JobOverlay')||document;
    scope.querySelectorAll?.('[data-edit-job]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();openJobEditor(button.dataset.editJob);});
    scope.querySelectorAll?.('[data-open-budget]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();openBudgetModal(button.dataset.openBudget);});
    scope.querySelectorAll?.('[data-edit-invoice]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();openFinancialEditor(button.dataset.editInvoice,'factura');});
    scope.querySelectorAll?.('[data-v09139-edit-cert]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();openFinancialEditor(button.dataset.v09139EditCert,'certificacio');});
    scope.querySelectorAll?.('[data-v09139-new-budget-job]').forEach(button=>button.onclick=()=>{state.v09139ReturnJobId=button.dataset.v09139NewBudgetJob;state.v09139ReturnJobTab='pressupost';openBudgetModal('__new');});
    scope.querySelectorAll?.('[data-v09139-create-invoice]').forEach(button=>button.onclick=()=>openInvoiceFromBudget(button,'factura'));
    scope.querySelectorAll?.('[data-v09139-create-cert]').forEach(button=>button.onclick=()=>openInvoiceFromBudget(button,'certificacio'));
    scope.querySelectorAll?.('[data-v09139-new-invoice]').forEach(button=>button.onclick=()=>openFinancialEditor('', 'factura',button.dataset.v09139JobId||state.selectedJobId,button.dataset.v09139BudgetId||''));
    scope.querySelectorAll?.('[data-v09139-import-invoice]').forEach(button=>button.onclick=()=>openInvoiceImport(button.dataset.v09139JobId||state.selectedJobId));
    scope.querySelectorAll?.('[data-v09139-create-cert]:not([data-v09139-budget-id])').forEach(button=>button.onclick=()=>openFinancialEditor('', 'certificacio',state.selectedJobId,''));
    scope.querySelectorAll?.('[data-v09136-save-invoice]').forEach(button=>button.onclick=event=>{event.preventDefault();const row=button.closest('tr');teimor09136SaveInvoiceRelation(button.dataset.v09136SaveInvoice,row);});
    scope.querySelectorAll?.('[data-v09139AttachmentForm],#v09139AttachmentForm').forEach(form=>form.onsubmit=saveJobAttachment);
    const attachmentForm=scope.querySelector?.('#v09139AttachmentForm'); if(attachmentForm) attachmentForm.onsubmit=saveJobAttachment;
    scope.querySelectorAll?.('[data-download-attachment]').forEach(button=>button.onclick=()=>downloadAttachment(button.dataset.downloadAttachment));
    scope.querySelectorAll?.('[data-v09139-delete-attachment]').forEach(button=>button.onclick=()=>deleteJobAttachment(button.dataset.v09139DeleteAttachment,button.dataset.v09139JobId,state.obresJobTab||'documentacio'));
    const hourForm=scope.querySelector?.('#v09139HourForm'); if(hourForm) hourForm.onsubmit=saveHour;
    const materialForm=scope.querySelector?.('#v09139MaterialForm'); if(materialForm) materialForm.onsubmit=saveMaterial;
    scope.querySelectorAll?.('[data-v09139-detect-materials]').forEach(button=>button.onclick=()=>detectJobMaterials(button.dataset.v09139DetectMaterials));
    scope.querySelectorAll?.('[data-v09139-delete-hour]').forEach(button=>button.onclick=()=>deleteHour(button.dataset.v09139DeleteHour,button.dataset.v09139JobId));
    scope.querySelectorAll?.('[data-v09139-delete-material]').forEach(button=>button.onclick=()=>deleteMaterial(button.dataset.v09139DeleteMaterial,button.dataset.v09139JobId));
    return result;
  };
  renderObresV0913=function(){
    try{ return teimor09136RenderObres(false); }
  ...
  /* Inici més net: els accessos ja són al menú lateral; aquí només queden
     els indicadors i les dades recents. */
  renderDashboard=function(){
    ensureShape();
    setHeader('Inici','Resum ràpid. El menú lateral és el punt únic d’accés a cada àrea.');
    const totalBudget=data.budgets.reduce((sum,item)=>sum+budgetBase(item),0);
    const totalInvoice=data.invoices.reduce((sum,item)=>sum+invoiceTotal(item),0);
    const recentJobs=[...data.jobs].sort((a,b)=>Number(b.year||0)-Number(a.year||0)).slice(0,8);
    setContent('<div class="grid four"><div class="kpi"><span>Clients</span><strong>'+data.clients.length+'</strong></div><div class="kpi"><span>Obres</span><strong>'+data.jobs.length+'</strong></div><div class="kpi"><span>Pressupostos</span><strong>'+data.budgets.length+'</strong></div><div class="kpi"><span>Factures</span><strong>'+data.invoices.length+'</strong></div></div><div class="card notice-blue"><strong>Flux recomanat:</strong> entra a <strong>Obres</strong>, obre una fitxa i treballa-hi el pressupost, les factures, certificacions, albarans, hores, materials i notes. Les pantalles del menú queden per als llistats i filtres globals.</div><div class="grid two"><div class="card"><h2>Resum econòmic</h2><p>Pressupostat s/IVA: <strong>'+money(totalBudget)+'</strong><br>Facturat IVA inclòs: <strong>'+money(totalInvoice)+'</strong><br>Partides de llibreria de referència: <strong>'+data.library.length+'</strong></p></div><div class="card"><h2>Obres recents</h2>'+table(['Any','Identificació','Client','Acció'],recentJobs.map(job=>'<tr><td>'+esc(job.year||'')+'</td><td><strong>'+esc(practicalKeyword(job,null))+'</strong><br><span class="muted">'+esc([workAddress(job,null),workCity(job,null)].filter(Boolean).join(' · '))+'</span></td><td>'+esc(clientName(job.clientId)||'Client pendent')+'</td><td><button class="primary small" data-v09139-dashboard-job="'+esc(job.id)+'">Obrir fitxa</button></td></tr>'))+'</div></div>');
    document.querySelectorAll('[data-v09139-dashboard-job]').forEach(button=>button.onclick=()=>{state.view='obres';state.selectedJobId=button.dataset.v09139DashboardJob;render();});
  };

  /* Obres és una pestanya real, no només un botó que canvia la vista. */
  const baseRenderV09139=render;
  render=function(){
    ensureShape();
    document.querySelectorAll('#tabs button').forEach(button=>button.classList.toggle('active',button.dataset.view===state.view));
    const result=baseRenderV09139();
    document.querySelectorAll('#tabs button').forEach(button=>button.classList.toggle('active',button.dataset.view===state.view));
    return result;
  };

  /* Delegació final per als controls creats dins de la fitxa. */
  const baseBindViewEventsV09139=bindViewEvents;
  bindViewEvents=function(){
    baseBindViewEventsV09139();
    document.querySelectorAll('[data-v09139-dashboard-job]').forEach(button=>button.onclick=()=>{state.view='obres';state.selectedJobId=button.dataset.v09139DashboardJob;render();});
    document.querySelectorAll('[data-v09139-create-cert]:not([data-v09139-budget-id])').forEach(button=>button.onclick=()=>openFinancialEditor('', 'certificacio',state.selectedJobId,''));
  };
...
(function(){
  ...
  function budgetScreenHtml(b,isNew){
    ...
    const needsReimport=!isNew && !/^9\.(?:15|16)\./.test(String(b.importParserVersion||''));
    const reimportNotice=needsReimport?'<div class="v0916-reimport-warning"><strong>Aquesta fitxa és d’una importació anterior.</strong> Per llegir de nou el 875 amb la nova interpretació, cal reimportar el mateix Excel. La reimportació actualitzarà les partides i les observacions sense duplicar el pressupost.</div>':'';
    return '<div class="v0916-budget-screen" id="v0916BudgetScreen" data-budget-id="'+esc(b.id||'')+'"><header class="v0916-budget-header"><div><span class="v0916-kicker">PRESSUPOST · EXCEL</span><h2>'+esc(isNew?'Nou pressupost':'Pressupost '+(budgetOldNumberV098(b)||b.number||b.id))+'</h2><p><strong>'+esc(client.name||'Client pendent')+'</strong><br>'+esc(b.title||job.title||'Concepte pendent de revisar')+(identity?'<br><span>'+esc(identity)+'</span>':'')+'</p></div><div class="v0916-budget-header-actions"><button class="ghost" type="button" data-preview-budget-modal="'+esc(b.id||'')+'" '+(isNew?'disabled':'')+'>Previsualitzar A4 / PDF</button><button class="ghost" type="button" data-v0916-budget-close>× Tancar</button></div></header><div class="v0916-budget-kpis"><div><span>Estat</span><strong>'+esc(statusLabel(b.status||'Esborrany','budget'))+'</strong></div><div><span>Partides</span><strong>'+esc((b.lines||[]).length)+'</strong></div><div><span>Base s/IVA</span><strong>'+money(budgetBase(b))+'</strong></div><div><span>Total IVA inclòs</span><strong>'+money(budgetTotal(b))+'</strong></div></div><nav class="v0916-budget-tabs" role="tablist"><button type="button" class="'+(firstTab==='partides'?'active':'')+'" data-v0916-budget-tab="partides">Partides del pressupost</button><button type="button" class="'+(firstTab==='capcalera'?'active':'')+'" data-v0916-budget-tab="capcalera">Capçalera i estat</button><button type="button" class="'+(firstTab==='observacions'?'active':'')+'" data-v0916-budget-tab="observacions">Observacions de l’Excel</button></nav><main class="v0916-budget-panels"><section class="v0916-budget-panel '+(firstTab==='partides'?'active':'')+'" data-v0916-budget-panel="partides"><div class="v0916-budget-panel-intro"><div><span class="v0916-kicker">LECTURA ESTRUCTURADA</span><h3>Partides, amidaments i preus</h3><p>Concepte curt i descripció llarga separats. Les files de mesurament de l’Excel s’han convertit en quantitat, unitat i preu unitari.</p></div><div class="v0916-source-badge">'+esc((b.lines||[]).length)+' línies de pressupost</div></div>'+reimportNotice+linesHtml+'</section><section class="v0916-budget-panel '+(firstTab==='capcalera'?'active':'')+'" data-v0916-budget-panel="capcalera">'+formHtml+'</section><section class="v0916-budget-panel '+(firstTab==='observacions'?'active':'')+'" data-v0916-budget-panel="observacions"><div class="v0916-budget-panel-intro"><div><span class="v0916-kicker">NOTA / OBSERVACIONS</span><h3>Text conservat de l’Excel</h3><p>Aquestes línies informatives no compten com a partides i es poden editar des de la capçalera del pressupost.</p></div></div>'+reimportNotice+budgetSourceHtml(b)+'<div class="card v0916-observation-card">'+budgetObservationHtml(b)+'</div></section></main></div>';
  }
...
/* V09.13.8 és un bloc històric que es carrega abans d’aquest tancament final
   i torna a embolcallar alguns listeners. Aquestes assignacions són la capa
   definitiva perquè la versió activa sigui sempre la V09.13.9. */
(function(){
  const baseJobFichaV09139=teimor09136JobFicha;
  teimor09136JobFicha=function(model){
    const tabs=[['resum','Resum'],['pressupost','Pressupostos'],['factures','Factures'],['certificacions','Certificacions'],['documentacio','Documentació'],['albarans','Albarans'],['notes','Notes'],['temps','Temps i materials'],['rendiment','Rendiment']];
    const tab=tabs.some(item=>item[0]===state.obresJobTab)?state.obresJobTab:'resum';
    let body=teimor09136SummaryTab(model);
    if(tab==='pressupost') body=teimor09136BudgetsTab(model);
    if(tab==='factures') body=teimor09136InvoicesTab(model);
    if(tab==='certificacions') body=teimor09136CertificationsTab(model);
    if(tab==='documentacio') body=teimor09136DocumentsTab(model,false);
    if(tab==='albarans') body=teimor09136DocumentsTab(model,true);
    if(tab==='notes') body=teimor09136NotesTab(model);
    if(tab==='temps') body=teimor09136TimeMaterialsTab(model);
  ...
  const baseConfirmFinancialV09139Final=confirmFinancialImportV0913;
  confirmFinancialImportV0913=async function(){
    const context=state.v09139ImportContext;
    await baseConfirmFinancialV09139Final();
    if(context){
      state.v09139ImportContext=null;
      returnToJob(context.jobId,'factures');
    }
  };
  const baseDownloadInvoiceV09139Final=downloadInvoiceFileV09132;
  downloadInvoiceFileV09132=async function(id){
    const invoice=byId(data.invoices,id);
    if(!invoice) return alert('No s’ha trobat la factura.');
    if(invoice.attachmentId) return baseDownloadInvoiceV09139Final(id);
    const budget=byId(data.budgets,invoice.budgetId)||{};
    const job=byId(data.jobs,invoice.jobId)||byId(data.jobs,budget.jobId)||{};
    const client=byId(data.clients,invoice.clientId)||{};
    const rows=(invoice.lines||invoice.invoiceLines||[]).length?(invoice.lines||invoice.invoiceLines).map(line=>'<tr><td>'+esc(line.concept||'')+'</td><td>'+esc(line.unit||'')+'</td><td>'+esc(line.qty||'')+'</td><td>'+money(line.unitPrice)+'</td><td>'+money(line.total||num(line.qty)*num(line.unitPrice))+'</td></tr>').join(''):'<tr><td>'+esc(invoice.concept||'Factura')+'</td><td>ud</td><td>1</td><td>'+money(invoiceBase(invoice))+'</td><td>'+money(invoiceBase(invoice))+'</td></tr>';
    const html='<!doctype html><html lang="ca"><head><meta charset="utf-8"><title>Factura '+esc(invoice.number||invoice.id)+'</title><style>'+previewCss()+'</style></head><body><div class="preview-toolbar actions"><button class="primary" onclick="window.print()">Imprimir / guardar PDF</button></div><div class="a4-sheet"><div class="preview-colorbar"></div><h1>Factura '+esc(invoice.number||invoice.id)+'</h1><p><strong>Client:</strong> '+esc(client.name||invoice.clientSnapshot?.name||'Client pendent')+'<br><strong>Obra:</strong> '+esc(job.keyword||job.title||budget.title||'')+' · '+esc(job.address||job.workAddress||'')+'<br><strong>Pressupost:</strong> '+esc(budget.number||'')+'<br><strong>Data:</strong> '+dateDisplay(invoice.date)+'</p><table class="preview-table"><thead><tr><th>Concepte</th><th>Ut.</th><th>Quant.</th><th>Preu/ut.</th><th>Total</th></tr></thead><tbody>'+rows+'</tbody></table><div class="preview-totals"><div>Base s/IVA: <strong>'+money(invoiceBase(invoice))+'</strong></div><div>IVA '+num(invoice.iva).toFixed(2)+'%: <strong>'+money(invoiceTotal(invoice)-invoiceBase(invoice))+'</strong></div><div>Total: <strong>'+money(invoiceTotal(invoice))+'</strong></div></div></div></body></html>';
    const blob=new Blob([html],{type:'text/html;charset=utf-8'});
    const url=URL.createObjectURL(blob); const link=document.createElement('a'); link.href=url; link.download='factura_'+(invoice.number||invoice.id)+'.html'; document.body.appendChild(link); link.click(); link.remove(); setTimeout(()=>URL.revokeObjectURL(url),1000);
  };
  data.meta=data.meta||{};
  data.meta.version='9.13.9-fitxa-autocontinguda-flux-obra';
  renderObresV0913=function(){
    try{ return teimor09136RenderObres(false); }
    catch(error){ console.error('No s’ha pogut obrir la fitxa d’obra:',error); return teimor09132RenderObresFallback(error); }
  };
...
/* =========================================================
   ...
   - Identificació d'obra separada de l'adreça real.
   - Les línies Nota/Observacions i les línies de mesurament no entren
     com a partides del pressupost.
   - Les observacions per defecte es poden editar a Configuració.
   - Temps i materials s'obren en una pantalla superior amb resum per dia.
   ========================================================= */
(function(){
  const VERSION='9.15.0-importacio-pantalles-superposades';
  const arr=value=>Array.isArray(value)?value:[];
  const tx=value=>String(value??'').replace(/\s+/g,' ').trim();
  const key=value=>strip(value||'').replace(/\s+/g,' ').trim();
  const todayKey=()=>typeof today==='function'?today():new Date().toISOString().slice(0,10);
  const fallbackObservations="Revisar PU abans d'enviar.\nPendent de valoració actualitzada.";

  function splitNotes(value){
    if(Array.isArray(value)) return value.flatMap(splitNotes);
    return String(value??'').split(/\r?\n+/).map(tx).filter(Boolean);
  }
  ...
  function generatedImportNote(value){
    const normalized=key(value);
    if(!normalized) return true;
    return /^(?:pressupost importat(?:\.|:)|pressupost importat\. revisa|importada autom[aà]ticament des d['’]excel|client detectat autom[aà]ticament|client llegit exclusivament|treballs separats per marcador|revisar pu abans d['’]enviar|pendent de valoraci[oó] actualitzada)/i.test(normalized);
  }
  function noteKey(value){
    return key(String(value??'').replace(/^\s*[.*•·-]\s*/,'')).replace(/[.;:]+$/,'');
  }
  function notePayload(value){
    const text=tx(value);
    if(!text || /^(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?)$/i.test(text)) return '';
    const clean=text.replace(/^\s*(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?)\s*[:=\-]?\s*/i,'').trim();
    return clean ? (/^[.*•·-]\s*/.test(clean) ? '• '+clean.replace(/^[.*•·-]\s*/,'') : clean) : '';
  }
  function extractExcelObservationNotes(flat){
    const result=[];
    const add=value=>{ const note=notePayload(value); if(note) result.push(note); };
    const bySheet={};
    arr(flat).forEach(record=>{ (bySheet[record.sheet] ||= []).push(record); });
    Object.values(bySheet).forEach(rows=>{
      rows.sort((a,b)=>a.rowIndex-b.rowIndex);
      let active=false;
      let emptyRows=0;
      for(const record of rows){
        const cells=(record.raw||[]).map(tx).filter(Boolean);
        if(!cells.length){
          if(active && ++emptyRows>=2) active=false;
          continue;
        }
        const markerIndex=cells.findIndex(cell=>/^(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?)\b/i.test(cell));
        if(markerIndex>=0){
          active=true; emptyRows=0;
          const inlineNote=cells[markerIndex].replace(/^\s*(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?)\s*[:=\-]?\s*/i,'');
          add(inlineNote);
          cells.slice(markerIndex+1).forEach(add);
          continue;
        }
        if(!active) continue;
        const joined=cells.join(' ');
        const structural=cells.some(cell=>/^(?:data|fecha|pressupost|presupuesto|obra|concepte|concepto|amidaments?|medicions?|treballs?|trabajos|partides?|subtotal|base imposable|base imponible|iva|total)\b/i.test(cell));
        const hasBullet=cells.some(cell=>/^\s*[.*•·-]\s*/.test(cell));
        if(structural && !hasBullet){ active=false; continue; }
        cells.forEach(add);
      }
    });
    return uniqueNotes(result);
  }
  function ensureCollections(){
    data.meta=data.meta||{};
    data.settings=data.settings||{};
    if(!tx(data.settings.defaultObservations)) data.settings.defaultObservations=fallbackObservations;
    data.jobs=arr(data.jobs);
    data.budgets=arr(data.budgets);
    data.clients=arr(data.clients);
    data.hores=arr(data.hores);
    data.materials=arr(data.materials);
    data.attachments=arr(data.attachments);
    data.agenda=arr(data.agenda);
  }
  ensureCollections();
  ...
  function genericText(value){
    const normalized=key(value);
    return !normalized ||
      /^(?:obra|obra sense nom|feina|feina sense nom|pressupost|pressupost importat|concepte pendent|concepte pendent de revisar|factura importada|certificacio importada|pendent de revisar)$/.test(normalized);
  }
  function isBadAddress(value){
    const text=tx(value);
    if(!text) return true;
    if(text.length>180) return true;
    return /\b(?:partid(?:a|es)|unitat(?:s)?|quantitat|amidament|mesurament|medici[oó]|preu|import|descripci[oó]|concepte|treballs?|total|iva|base)\b/i.test(text);
  }
  function looksAddressLocal(value){
    const text=tx(value);
    if(!text) return false;
    try{ if(typeof looksLikeAddress==='function' && looksLikeAddress(text)) return true; }catch(error){}
    return /(?:c\/|carrer|calle|avinguda|avenida|av\.?|avda\.?|passeig|pla[cç]a|plaza|rambla|carretera|ctra\.?|urbanitzaci[oó]|pol[ií]gon|cam[ií]|travessera)/i.test(text) ||
      /\b\d{1,5}\b/.test(text);
  }
  function addressCandidate(value,explicit){
    let text=tx(value).replace(/^[|;,\s]+|[|;,\s]+$/g,'');
    text=text.replace(/^(?:adre[cç]a(?:\s+de)?(?:\s+l[ao])?\s+obra|direcci[oó]n?(?:\s+de)?(?:\s+l[ao])?\s+obra|empla[cç]ament|emplazamiento)\s*[:=\-]\s*/i,'');
    if(!text || isBadAddress(text)) return '';
    if(explicit || looksAddressLocal(text) || /^(?:zona|pol[ií]gon|urbanitzaci[oó]|urbanizacion|finca|nau|edifici|edificio)\b/i.test(text)) return text.slice(0,180).trim();
    return '';
  }
  function extractAddressFromText(value){
    const raw=String(value??'');
    const explicit=raw.match(/(?:^|\n|\|)\s*(?:obra|adre[cç]a(?:\s+de)?(?:\s+l[ao])?\s+obra|direcci[oó]n?(?:\s+de)?(?:\s+l[ao])?\s+obra|empla[cç]ament|emplazamiento)\s*[:=\-]\s*([^\n|]+)/i);
    if(explicit){
      const candidate=addressCandidate(explicit[1],true);
      if(candidate) return candidate;
    }
    const obraOnly=raw.match(/(?:^|\n|\|)\s*obra\s+([^\n|]+)/i);
    if(obraOnly){
      const candidate=addressCandidate(obraOnly[1],true);
      if(candidate) return candidate;
    }
    return '';
  }
  function explicitWorkAddress(flat){
    for(const record of arr(flat)){
      const row=record.raw||[];
      for(let index=0;index<row.length;index++){
        const cell=tx(row[index]);
        if(!cell) continue;
        const inline=extractAddressFromText(cell);
        if(inline) return inline;
        const label=/^(?:obra|obra\s*[/:-]\s*(?:adre[cç]a|direcci[oó])|adre[cç]a(?:\s+de)?(?:\s+l[ao])?\s+obra|direcci[oó]n?(?:\s+de)?(?:\s+l[ao])?\s+obra|empla[cç]ament|emplazamiento|ubicaci[oó]n\s+obra)$/i.test(cell);
        if(!label) continue;
        const afterLabel=(row||[]).slice(index+1).map(tx).filter(Boolean);
        const candidates=[row[index+1],afterLabel[0],row[index-1]];
        for(const next of arr(flat)){
          if(next.sheet===record.sheet && next.rowIndex===record.rowIndex+1){
            candidates.push((next.raw||[])[index],(next.raw||[])[index+1]);
          }
        }
        for(const candidate of candidates){
          const address=addressCandidate(candidate,true);
  ...
  function workAddressFor(job,budget,client){
    const candidates=[
      extractAddressFromText(job?.workAddress),extractAddressFromText(job?.address),
      extractAddressFromText(budget?.workAddress),extractAddressFromText(budget?.address),
      job?.workAddress,job?.address,budget?.workAddress,budget?.address,client?.workAddress
    ];
    return candidates.map(value=>addressCandidate(value,false)).find(Boolean)||'';
  }
  function workCityFor(job,budget,address){
    const direct=[job?.workCity,job?.city,budget?.workCity,budget?.city].map(tx).find(value=>value&&!/^(?:concepte|concepto|obra|pendent)/i.test(value));
    if(direct) return direct;
    try{
      const postal=tx(job?.workPostalCode||job?.postalCode||budget?.workPostalCode||budget?.postalCode);
      if(typeof teimor0911CityFromText==='function') return tx(teimor0911CityFromText(address,postal));
    }catch(error){}
    return '';
  }
  function cleanKeyword(value,address){
    let text=tx(value).replace(/^\s*[.*•·-]\s*/,'');
    text=text.replace(/^(?:pressupost|presupuesto|concepte|concepto|feina|treball|trabajo)\s*(?:n[úu]m(?:ero)?\.?\s*[:#-]?\s*[\w/-]+\s*[-:])?\s*/i,'');
    text=text.replace(/^\s*obra\s*[:\-]\s*/i,'').trim();
    if(!text || genericText(text) || (address && key(text)===key(address)) || /^(?:adre[cç]a|direcci[oó]n?|unitat|unidad|quantitat|cantidad|amidament|mesurament|preu|precio|import|importe|total|iva|base)\b/i.test(text)) return '';
    if(text.length>110) text=(text.split(/[.;!?]\s+/)[0]||text).slice(0,110).trim();
    return text;
  }
  function rawKeywordFor(job,budget,address){
    const candidates=[job?.keyword,budget?.keyword,budget?.workKeyword,job?.displayKeyword,job?.title,budget?.title];
    for(const candidate of candidates){
      const value=cleanKeyword(candidate,address);
      if(value) return value;
    }
    return '';
  }
  function practicalIdentity(job,budget,client){
    const address=workAddressFor(job,budget,client);
    const keyword=rawKeywordFor(job,budget,address);
    return {address,city:workCityFor(job,budget,address),keyword:keyword||address||tx(client?.name)||'Obra sense identificació',hasKeyword:!!keyword};
  }
  function observationText(line){
    const concept=tx(line?.concept||line?.description||'');
    const desc=tx(line?.longDesc||line?.notes||line?.observation||'');
    const conceptOnly=/^(?:nota|notes|observaci[oó](?:n|ons)?|observacions?|condici[oó](?:n|ons)?|exclusions?|comentari(?:s)?)\s*[:=\-]?$/i.test(concept);
    const value=conceptOnly?desc:[concept,desc].filter(Boolean).join(' ');
    return value.replace(/^\s*(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?)\s*[:=\-]\s*/i,'').trim();
  }
  function isObservationLine(line){
    const raw=tx([line?.chapter,line?.code,line?.concept,line?.longDesc,line?.notes,line?.observation].filter(Boolean).join(' '));
    if(!raw || line?.genericWork) return false;
    const explicitNote=/(?:^|\s)(?:nota|notes|observaci[oó](?:n|ons)?|condici[oó](?:n|ons)?|exclusions?|comentari(?:s)?)\s*[:=\-]?\s*/i.test(raw);
    if(explicitNote) return true;
    const noPrice=!num(line?.unitPrice)&&!num(line?.total);
    if(!noPrice) return false;
    return /\b(?:no inclou|no incluye|excl[oò]s|excluido|exclusi[oó]|a c[aà]rrec del client|a cargo del cliente|aigua\s*,?\s*llum|permisos?\s+(?:i|y)\s+|advertiment|tenir en compte)\b/i.test(raw) ||
      /^gen[-\s]?\d+\b/i.test(tx(line?.code)) && /\b(?:aigua|llum|permis|client|inclou|incluye|excl)/i.test(raw);
  }
  function isMeasurementLine(line){
    const raw=tx([line?.concept,line?.longDesc,line?.notes].filter(Boolean).join(' '));
    if(!raw || line?.genericWork) return false;
    const explicitMeasurement=/^(?:l|a|h|llargada|longitud|amplada|anchura|al[cç]ada|altura|superf[ií]cie|mesurament|medici[oó]n?)\s*[:=]/i.test(raw) ||
      /\b(?:l|a|h)\s*=\s*\d+(?:[.,]\d+)?/i.test(raw) ||
      /^\s*\d+(?:[.,]\d+)?\s*[x×*]\s*\d+(?:[.,]\d+)?(?:\s*[x×*]\s*\d+(?:[.,]\d+)?)?\s*(?:=|m[²2³3]?|\s*$)/i.test(raw);
    return explicitMeasurement && !num(line?.unitPrice)&&!num(line?.total);
  }
  function cleanLines(lines){
    ...
    arr(lines).forEach(line=>{
      if(isObservationLine(line)){
        const note=observationText(line);
      ...
      const hasText=tx([line?.concept,line?.longDesc,line?.chapter,line?.code].filter(Boolean).join(' '));
      if(hasText || num(line?.qty)||num(line?.unitPrice)||num(line?.total)) kept.push(line);
    });
    return {kept,observations:uniqueNotes(observations),measurements};
  }
  function applyBudgetLineCleanup(budget,includeDefaults){
    if(!budget) return {changed:false,observations:[],measurements:0};
    const before=arr(budget.lines);
    const result=cleanLines(before);
    let changed=result.kept.length!==before.length;
    if(changed) budget.lines=result.kept.map(line=>({...line,id:line.id||uid('LIN')}));
    const existingNotes=splitNotes(budget.notes).filter(note=>!generatedImportNote(note));
    const existingObservations=splitNotes(budget.observations);
    const nextNotes=uniqueNotes(existingNotes.concat(existingObservations,result.observations,includeDefaults?splitNotes(data.settings.defaultObservations):[])).filter(note=>!generatedImportNote(note));
    if(nextNotes.join('\n')!==existingNotes.join('\n')){
      budget.notes=nextNotes.join('\n');
      changed=true;
    }
    if(result.observations.length){
      const next=uniqueNotes(existingObservations.concat(result.observations));
      if(JSON.stringify(next)!==JSON.stringify(existingObservations)){ budget.observations=next; changed=true; }
    }
    budget.parseStats={...(budget.parseStats||{}),observationLines:(budget.observations||[]).length,measurementLinesRemoved:num(budget.parseStats?.measurementLinesRemoved)+result.measurements};
    return {changed,observations:result.observations,measurements:result.measurements};
  }
  function normalizeParsedImport(parsed,fileName,arrayBuffer){
    if(!parsed) return parsed;
    ensureCollections();
    let flat=[];
    try{ flat=typeof teimor0913v2ReadFlat==='function'?teimor0913v2ReadFlat(arrayBuffer):[]; }catch(error){ flat=[]; }
    const budget=parsed.budget=parsed.budget||{};
    const job=parsed.job=parsed.job||{};
    const client=parsed.client=parsed.client||{};
    const explicit=explicitWorkAddress(flat);
    const identity=practicalIdentity(job,budget,client);
    const address=explicit||identity.address;
    if(address){
    ...
    const initialCleaned=cleanLines(sourceLines);
    const observationKeys=excelObservations.map(noteKey).filter(Boolean);
    const noteLines=initialCleaned.kept.filter(line=>{
      const lineText=noteKey(observationText(line));
      return lineText && observationKeys.some(note=>lineText===note || (lineText.length>12&&note.includes(lineText)) || (note.length>12&&lineText.includes(note)));
    });
    const cleaned={
      kept:initialCleaned.kept.filter(line=>!noteLines.includes(line)),
      observations:uniqueNotes(initialCleaned.observations.concat(noteLines.map(observationText),excelObservations)),
      measurements:initialCleaned.measurements
    };
    budget.importParserVersion='9.16.0-lector-875-estats-fitxa-completa';
    budget.lines=cleaned.kept.map(line=>({...line,id:line.id||uid('LIN')}));
    parsed.items=cleaned.kept.map(line=>({...line,id:line.id||uid('LIN'),origin:line.origin||fileName,sourceBudget:line.sourceBudget||budget.number||fileName}));
    const observations=uniqueNotes(splitNotes(budget.observations).concat(cleaned.observations));
    ...
    if(cleaned.observations.length||cleaned.measurements){
      ...
      if(cleaned.measurements) parsed.warnings.push(fileName+': '+cleaned.measurements+' línia/es de mesurament descartades perquè no eren partides.');
    }
    if(excelObservations.length) parsed.warnings.push(fileName+': s’han llegit '+excelObservations.length+' observació/ons directament del bloc «NOTA/Observacions» de l’Excel.');
    if(explicit) parsed.warnings=arr(parsed.warnings).concat(fileName+': adreça de l’obra llegida del camp «Obra» / «Adreça de l’obra».');
    parsed.warnings=[...new Set(arr(parsed.warnings))];
    return parsed;
  }

  const baseParseV09140=parseWorkbook;
  parseWorkbook=function(fileName,arrayBuffer){
    const parsed=baseParseV09140(fileName,arrayBuffer);
    return normalizeParsedImport(parsed,fileName,arrayBuffer);
  };

  function migrateExistingData(){
    ensureCollections();
    if(data.meta.v09140Migration===VERSION) return;
    let changed=false;
    data.budgets.forEach(budget=>{
      const result=applyBudgetLineCleanup(budget,false);
      if(result.changed) changed=true;
    });
    data.jobs.forEach(job=>{
      const budget=data.budgets.find(item=>item.jobId===job.id)||null;
      const client=job.clientId?byId(data.clients,job.clientId):null;
      const identity=practicalIdentity(job,budget,client);
      if(identity.address && (tx(job.address)!==identity.address || tx(job.workAddress)!==identity.address)){
        job.address=identity.address; job.workAddress=identity.address; changed=true;
      }
      if(identity.city && !tx(job.workCity||job.city)){ job.city=identity.city; job.workCity=identity.city; changed=true; }
      if(budget && identity.address && !tx(budget.workAddress)){ budget.workAddress=identity.address; changed=true; }
    });
    data.meta.v09140Migration=VERSION;
    data.meta.version=VERSION;
    if(changed) saveData();
  }
  migrateExistingData();

  function applyModelIdentity(model){
    const identity=practicalIdentity(model.job,model.budgets?.[0],model.client);
    model.address=identity.address;
    model.city=identity.city;
    model.keyword=identity.keyword;
    model.keywordNeedsReview=!identity.hasKeyword;
    model.addressNeedsReview=!identity.address;
    model.blob=strip([model.blob,identity.keyword,identity.address,identity.city,model.client?.name].join(' '));
  }
  const baseBuildModelsV09140=teimor09136BuildModels;
  teimor09136BuildModels=function(){
    const built=baseBuildModelsV09140();
    built.models.forEach(applyModelIdentity);
    return built;
  };
  ...
  teimor09136JobRows=function(models){
    ...
    return models.map(model=>{
      const address=[model.address,model.city].filter(Boolean).join(' · ');
      const idLabel=model.keyword||'Obra sense identificació';
      return '<tr class="v09140-job-row" data-v09136-obres-row="1"><td>'+esc(model.year||'')+'</td><td><div class="v09140-work-id"><strong>'+esc(idLabel)+'</strong>'+(model.keywordNeedsReview?'<span class="v09140-review-badge">Revisar identificació</span>':'')+'</div><span class="v09140-work-address">'+esc(address||'Adreça pendent de completar')+'</span></td><td>'+esc(model.client?.name||clientName(model.job.clientId)||'Client pendent')+'</td><td class="num">'+model.budgets.length+'</td><td class="num">'+model.invoices.length+'</td><td class="num">'+model.certifications.length+'</td><td class="num">'+money(model.budgetTotal)+'</td><td class="num">'+money(model.invoiceBase)+'</td><td>'+statusPill(model.job.status||'')+'</td><td class="nowrap"><button class="primary small" data-v0913-trace-job="'+esc(model.job.id)+'" data-v09136-open-job="'+esc(model.job.id)+'">Obrir fitxa</button> <button class="ghost small" data-edit-job="'+esc(model.job.id)+'">Editar obra</button></td></tr>';
    }).join('');
  };

  const baseOpenBudgetV09140=openBudgetModal;
  openBudgetModal=function(id=''){
    const result=baseOpenBudgetV09140(id);
    if(id==='__new'||!id){
      const notes=document.querySelector('#budgetForm [name="notes"]');
      if(notes&&!tx(notes.value)) notes.value=String(data.settings.defaultObservations||'');
    }
    return result;
  };

  const baseRenderSettingsV09140=renderSettings;
  renderSettings=function(){
    baseRenderSettingsV09140();
    const content=document.getElementById('content');
    if(!content) return;
    content.insertAdjacentHTML('beforeend','<div class="card v09140-settings-card"><div class="v09140-section-kicker">PRESSUPOSTOS NOUS</div><h2>Observacions per defecte</h2><p class="muted">Aquest text s’afegeix als pressupostos nous. En els Excel importats es conserven només les notes que hi ha al bloc «NOTA/Observacions» del fitxer.</p><form id="v09140ObservationsForm"><label>Observacions inicials<textarea name="defaultObservations" class="v09140-settings-textarea">'+esc(data.settings.defaultObservations||'')+'</textarea></label><div class="actions"><button class="primary" type="submit">Guardar observacions</button></div></form></div>');
    const form=document.getElementById('v09140ObservationsForm');
    if(form) form.onsubmit=event=>{
      event.preventDefault();
      data.settings.defaultObservations=formObj(form).defaultObservations||'';
      saveData();
      alert('Observacions per defecte guardades.');
      renderSettings();
    };
  };

  function hourCost(item){
    return num(item?.total)||Number((num(item?.officialHours)*num(item?.officialRate)+num(item?.laborerHours)*num(item?.laborerRate)).toFixed(2));
  }
  function materialCost(item){
    return num(item?.total)||Number((num(item?.qty||item?.quantity)*num(item?.unitPrice||item?.price)).toFixed(2));
  }
  function dateKeyFor(item){
    const value=tx(item?.date||item?.day||item?.createdAt||'');
    if(!value) return '';
    try{ return parseDateValue(value)||value.slice(0,10); }catch(error){ return value.slice(0,10); }
  }
  function timeBoardData(jobId){
    const hours=arr(data.hores).filter(item=>item.jobId===jobId);
    const materials=arr(data.materials).filter(item=>item.jobId===jobId);
    const keys=[...new Set(hours.concat(materials).map(dateKeyFor))].sort((a,b)=>String(b).localeCompare(String(a)));
    if(!keys.length) keys.push('');
    return {hours,materials,keys};
  }
  function dayLabel(value){ return value?dateDisplay(value):'Data pendent'; }
  function dayCardHtml(day,hours,materials,jobId){
    const officialHours=hours.reduce((sum,item)=>sum+num(item.officialHours),0);
    const officialCost=hours.reduce((sum,item)=>sum+num(item.officialHours)*num(item.officialRate),0);
    const laborerHours=hours.reduce((sum,item)=>sum+num(item.laborerHours),0);
    const laborerCost=hours.reduce((sum,item)=>sum+num(item.laborerHours)*num(item.laborerRate),0);
    const materialTotal=materials.reduce((sum,item)=>sum+materialCost(item),0);
    const total=officialCost+laborerCost+materialTotal;
    const hourRows=hours.length?hours.map(item=>'<tr><td><strong>'+esc(item.concept||'Jornada d’obra')+'</strong><small>'+esc(item.notes||'')+'</small></td><td class="num">'+num(item.officialHours).toFixed(2)+'</td><td class="num">'+money(item.officialRate)+'</td><td class="num">'+num(item.laborerHours).toFixed(2)+'</td><td class="num">'+money(item.laborerRate)+'</td><td class="num">'+money(hourCost(item))+'</td><td><button class="danger small" data-v09140-delete-hour="'+esc(item.id)+'" data-v09140-job-id="'+esc(jobId)+'">Eliminar</button></td></tr>').join(''):'<tr><td colspan="7" class="muted">No hi ha hores aquest dia.</td></tr>';
    const materialRows=materials.length?materials.map(item=>'<tr><td><strong>'+esc(item.concept||item.name||'Material')+'</strong><small>'+esc(item.source||item.notes||'')+'</small></td><td>'+esc(item.unit||'ud')+'</td><td class="num">'+num(item.qty||item.quantity).toFixed(3)+'</td><td class="num">'+money(item.unitPrice||item.price)+'</td><td class="num">'+money(materialCost(item))+'</td><td><button class="danger small" data-v09140-delete-material="'+esc(item.id)+'" data-v09140-job-id="'+esc(jobId)+'">Eliminar</button></td></tr>').join(''):'<tr><td colspan="6" class="muted">No hi ha materials aquest dia.</td></tr>';
    return '<section class="v09140-day-card"><div class="v09140-day-head"><div><span class="v09140-day-label">JORNADA</span><h3>'+dayLabel(day)+'</h3></div><strong>'+money(total)+'</strong></div><div class="v09140-day-metrics"><div><span>Oficial</span><strong>'+officialHours.toFixed(2)+' h</strong><small>'+money(officialCost)+'</small></div><div><span>Manobre</span><strong>'+laborerHours.toFixed(2)+' h</strong><small>'+money(laborerCost)+'</small></div><div><span>Materials</span><strong>'+money(materialTotal)+'</strong><small>'+materials.length+' línies</small></div><div class="is-total"><span>Total jornada</span><strong>'+money(total)+'</strong><small>guardat</small></div></div><div class="v09140-board-subtitle">Hores i personal</div><div class="table-wrap"><table class="v09140-board-table"><thead><tr><th>Concepte</th><th>Oficial h</th><th>€/h</th><th>Manobre h</th><th>€/h</th><th>Total</th><th></th></tr></thead><tbody>'+hourRows+'</tbody></table></div><div class="v09140-board-subtitle">Materials</div><div class="table-wrap"><table class="v09140-board-table"><thead><tr><th>Material</th><th>Ut.</th><th>Quant.</th><th>Preu/ut.</th><th>Total</th><th></th></tr></thead><tbody>'+materialRows+'</tbody></table></div></section>';
  }
  function timeBoardHtml(jobId){
    const job=byId(data.jobs,jobId)||{};
    const client=byId(data.clients,job.clientId)||{};
    const info=timeBoardData(jobId);
    const dayHtml=info.keys.map(day=>dayCardHtml(day,info.hours.filter(item=>dateKeyFor(item)===day),info.materials.filter(item=>dateKeyFor(item)===day),jobId)).join('');
    const totalHours=info.hours.reduce((sum,item)=>sum+num(item.officialHours)+num(item.laborerHours),0);
    const totalMaterials=info.materials.reduce((sum,item)=>sum+materialCost(item),0);
    const totalHoursCost=info.hours.reduce((sum,item)=>sum+hourCost(item),0);
    return '<div class="v09140-timeboard"><div class="v09140-board-header"><div><span class="v09140-section-kicker">FITXA D’OBRA</span><h2>Temps i materials</h2><p>'+esc(practicalIdentity(job,null,client).keyword)+'<br><span class="muted">'+esc([job.address||job.workAddress,job.city||job.workCity].filter(Boolean).join(' · '))+'</span></p></div><div class="v09140-board-total"><span>Cost acumulat</span><strong>'+money(totalHoursCost+totalMaterials)+'</strong><small>guardat localment</small></div></div><div class="v09140-board-actions"><button class="ghost" type="button" data-v09140-close-timeboard>← Tornar a la fitxa</button><span class="v09140-saved-dot">● Canvis guardats automàticament</span></div><div class="grid three v09140-board-kpis"><div class="kpi"><span>Hores totals</span><strong>'+totalHours.toFixed(2)+' h</strong></div><div class="kpi"><span>Cost personal</span><strong>'+money(totalHoursCost)+'</strong></div><div class="kpi"><span>Materials</span><strong>'+money(totalMaterials)+'</strong></div></div><div class="v09140-board-layout"><div class="v09140-days">'+dayHtml+'</div><aside class="v09140-entry-panel"><div class="v09140-entry-card"><div class="v09140-section-kicker">NOVA LÍNIA</div><h3>Afegir jornada</h3><form id="v09140HourBoardForm" class="v09140-board-form" data-job-id="'+esc(jobId)+'"><label>Dia<input name="date" type="date" value="'+todayKey()+'" required></label><label>Concepte<input name="concept" value="Jornada d’obra"></label><div class="grid two"><label>Oficial h<input name="officialHours" type="number" min="0" step="0.25" value="0"></label><label>€/h oficial<input name="officialRate" type="number" min="0" step="0.01" value="'+esc(job.officialRate||data.settings.officialHourRate||31)+'"></label><label>Manobre h<input name="laborerHours" type="number" min="0" step="0.25" value="0"></label><label>€/h manobre<input name="laborerRate" type="number" min="0" step="0.01" value="'+esc(job.laborerRate||data.settings.laborerHourRate||27)+'"></label></div><label>Notes<textarea name="notes" rows="2"></textarea></label><button class="primary full" type="submit">Guardar jornada</button></form></div><div class="v09140-entry-card"><div class="v09140-section-kicker">MATERIAL</div><h3>Afegir material</h3><form id="v09140MaterialBoardForm" class="v09140-board-form" data-job-id="'+esc(jobId)+'"><label>Dia<input name="date" type="date" value="'+todayKey()+'" required></label><label>Material / concepte<input name="concept" required placeholder="Rajola, morter, làmina..."></label><div class="grid two"><label>Unitat<input name="unit" value="ud"></label><label>Quantitat<input name="qty" type="number" min="0" step="0.001" value="1"></label><label>Preu/ut. €<input name="unitPrice" type="number" min="0" step="0.01" value="0"></label><label>Origen<input name="source" placeholder="Albarà / proveïdor"></label></div><label>Notes<textarea name="notes" rows="2"></textarea></label><button class="primary full" type="submit">Guardar material</button></form></div></aside></div></div>';
  }
  function markTimeModal(){
    const modal=document.getElementById('modal');
    if(!modal) return;
    modal.classList.add('v09140-time-modal');
    modal.querySelector('.modal-box')?.classList.add('v09140-time-modal-box');
  }
  const baseCloseModalV09140=closeModal;
  closeModal=function(){
    document.getElementById('modal')?.classList.remove('v09140-time-modal');
    document.getElementById('modal')?.querySelector('.modal-box')?.classList.remove('v09140-time-modal-box');
    return baseCloseModalV09140();
  };
  function closeTimeBoard(){ closeModal(); }
  function saveBoardHour(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const item={id:uid('HRS'),jobId:fields.jobId,date:fields.date||todayKey(),officialHours:num(fields.officialHours),officialRate:num(fields.officialRate),laborerHours:num(fields.laborerHours),laborerRate:num(fields.laborerRate),hours:num(fields.officialHours)+num(fields.laborerHours),total:Number((num(fields.officialHours)*num(fields.officialRate)+num(fields.laborerHours)*num(fields.laborerRate)).toFixed(2)),concept:fields.concept||'Jornada d’obra',notes:fields.notes||'',status:'Manual'};
    data.hores.push(item);
    saveData();
    openTimeBoard(item.jobId);
  }
  function saveBoardMaterial(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const item={id:uid('MAT'),jobId:fields.jobId,date:fields.date||todayKey(),concept:fields.concept||'Material',name:fields.concept||'Material',unit:fields.unit||'ud',qty:num(fields.qty),unitPrice:num(fields.unitPrice),total:Number((num(fields.qty)*num(fields.unitPrice)).toFixed(2)),source:fields.source||'Manual',notes:fields.notes||'',status:'Manual'};
    data.materials.push(item);
    saveData();
    openTimeBoard(item.jobId);
  }
  function deleteBoardHour(id,jobId){
    if(!confirm('Eliminar aquesta imputació d’hores?')) return;
    data.hores=data.hores.filter(item=>item.id!==id);
    saveData();
    openTimeBoard(jobId);
  }
  function deleteBoardMaterial(id,jobId){
    if(!confirm('Eliminar aquest material?')) return;
    data.materials=data.materials.filter(item=>item.id!==id);
    saveData();
    openTimeBoard(jobId);
  }
  function bindTimeBoard(){
    document.querySelectorAll('[data-v09140-close-timeboard]').forEach(button=>button.onclick=closeTimeBoard);
    const hourForm=document.getElementById('v09140HourBoardForm');
    if(hourForm) hourForm.onsubmit=saveBoardHour;
    const materialForm=document.getElementById('v09140MaterialBoardForm');
    if(materialForm) materialForm.onsubmit=saveBoardMaterial;
    document.querySelectorAll('[data-v09140-delete-hour]').forEach(button=>button.onclick=()=>deleteBoardHour(button.dataset.v09140DeleteHour,button.dataset.v09140JobId));
    document.querySelectorAll('[data-v09140-delete-material]').forEach(button=>button.onclick=()=>deleteBoardMaterial(button.dataset.v09140DeleteMaterial,button.dataset.v09140JobId));
  }
  function openTimeBoard(jobId){
    ensureCollections();
    const job=byId(data.jobs,jobId);
    if(!job) return alert('No s’ha trobat aquesta obra.');
    openModal(timeBoardHtml(jobId));
    markTimeModal();
    bindTimeBoard();
  }
  window.teimor09140OpenTimeBoard=openTimeBoard;

  const baseTimeTabV09140=teimor09136TimeMaterialsTab;
  teimor09136TimeMaterialsTab=function(model){
    const hours=arr(data.hores).filter(item=>item.jobId===model.job.id);
    const materials=arr(data.materials).filter(item=>item.jobId===model.job.id);
    const total=hours.reduce((sum,item)=>sum+hourCost(item),0)+materials.reduce((sum,item)=>sum+materialCost(item),0);
    const banner='<div class="v09140-time-launch"><div><span class="v09140-section-kicker">CONTROL DE COSTOS</span><h3>Temps i materials de l’obra</h3><p>Consulta ràpid i obre el quadre complet per introduir jornades, oficials, manobres i materials per dia.</p></div><div class="v09140-launch-right"><strong>'+money(total)+'</strong><button class="primary" type="button" data-v09140-open-timeboard="'+esc(model.job.id)+'">Obrir quadre de jornades</button></div></div>';
    return banner+baseTimeTabV09140(model);
  };
  const baseRenderObresV09140=teimor09136RenderObres;
  teimor09136RenderObres=function(prepareData){
    const result=baseRenderObresV09140(prepareData);
    const scope=document.getElementById('v09137JobOverlay')||document;
    scope.querySelectorAll?.('[data-v09140-open-timeboard]').forEach(button=>button.onclick=()=>openTimeBoard(button.dataset.v09140OpenTimeboard));
    return result;
  };
  renderObresV0913=function(){
    try{ return teimor09136RenderObres(false); }
  ...
  renderDashboard=function(){
    ...
    const totalBudget=data.budgets.reduce((sum,item)=>sum+budgetBase(item),0);
    const totalInvoice=data.invoices.reduce((sum,item)=>sum+invoiceTotal(item),0);
    const linked=data.invoices.filter(item=>item.jobId||item.budgetId).length;
    const recent=built.models.slice(0,6);
    setHeader('Inici','Un espai clar per controlar clients, pressupostos i cada obra.');
    setContent('<div class="v09140-welcome"><div><span class="v09140-section-kicker">TEIMOR · CENTRE DE CONTROL</span><h2>Treballa cada obra des d’un sol lloc</h2><p>Obre una fitxa, revisa el pressupost i continua amb factura, certificació, documentació, hores i materials.</p><div class="actions"><button class="primary" data-go="obres">Obrir obres</button><button class="v09140-welcome-ghost" data-go="budgets">Veure pressupostos</button></div></div><div class="v09140-welcome-side"><span>Obres actives</span><strong>'+built.models.length+'</strong><small>'+data.budgets.length+' pressupostos · '+data.invoices.length+' factures</small></div></div><div class="grid four"><div class="kpi"><span>Clients</span><strong>'+data.clients.length+'</strong></div><div class="kpi"><span>Obres</span><strong>'+built.models.length+'</strong></div><div class="kpi"><span>Pressupostos</span><strong>'+data.budgets.length+'</strong></div><div class="kpi '+(totalBudget-totalInvoice>=0?'good':'bad')+'"><span>Per facturar s/IVA</span><strong>'+money(totalBudget-totalInvoice)+'</strong></div></div><div class="v09140-dashboard-grid"><div class="card v09140-economy-card"><div class="v09140-section-kicker">RESUM ECONÒMIC</div><h2>Visió general</h2><div class="v09140-money-row"><span>Pressupostat s/IVA</span><strong>'+money(totalBudget)+'</strong></div><div class="v09140-money-row"><span>Facturat IVA inclòs</span><strong>'+money(totalInvoice)+'</strong></div><div class="v09140-money-row"><span>Factures relacionades</span><strong>'+linked+' / '+data.invoices.length+'</strong></div><div class="v09140-economy-footer"><span>Partides de referència</span><strong>'+data.library.length+'</strong></div></div><div class="card"><div class="toolbar"><div><div class="v09140-section-kicker">SEGUIMENT</div><h2>Obres recents</h2></div><button class="ghost small" data-go="obres">Veure totes</button></div>'+table(['Any','Identificació','Client','Acció'],recent.map(model=>'<tr><td>'+esc(model.year||'')+'</td><td><strong>'+esc(model.keyword)+'</strong><br><span class="muted">'+esc([model.address,model.city].filter(Boolean).join(' · '))+'</span></td><td>'+esc(model.client?.name||'Client pendent')+'</td><td><button class="primary small" data-v09140-dashboard-job="'+esc(model.job.id)+'">Obrir fitxa</button></td></tr>'))+'</div></div>');
    document.querySelectorAll('[data-v09140-dashboard-job]').forEach(button=>button.onclick=()=>{state.view='obres';state.selectedJobId=button.dataset.v09140DashboardJob;render();});
  };

  /* V09.15 · tauler de jornades per pantalles: cap formulari queda al lateral
     ni obliga a fer scroll per arribar als materials. Cada alta obre una
     pantalla superposada pròpia i el tauler conserva les pestanyes. */
  function timeBoardDayKeys(info,kind){
    const source=kind==='materials'?info.materials:info.hours;
    return info.keys.filter(day=>source.some(item=>dateKeyFor(item)===day));
  }
  function hoursDayCardHtml(day,hours,jobId){
    const officialHours=hours.reduce((sum,item)=>sum+num(item.officialHours),0);
    const officialCost=hours.reduce((sum,item)=>sum+num(item.officialHours)*num(item.officialRate),0);
    const laborerHours=hours.reduce((sum,item)=>sum+num(item.laborerHours),0);
    const laborerCost=hours.reduce((sum,item)=>sum+num(item.laborerHours)*num(item.laborerRate),0);
    const total=officialCost+laborerCost;
    const rows=hours.length?hours.map(item=>'<tr><td><strong>'+esc(item.concept||'Jornada d’obra')+'</strong><small>'+esc(item.notes||'')+'</small></td><td class="num">'+num(item.officialHours).toFixed(2)+'</td><td class="num">'+money(item.officialRate)+'</td><td class="num">'+num(item.laborerHours).toFixed(2)+'</td><td class="num">'+money(item.laborerRate)+'</td><td class="num">'+money(hourCost(item))+'</td><td><button class="danger small" data-v09140-delete-hour="'+esc(item.id)+'" data-v09140-job-id="'+esc(jobId)+'" data-v09140-board-tab="jornades">Eliminar</button></td></tr>').join(''):'<tr><td colspan="7" class="muted">No hi ha hores registrades aquest dia.</td></tr>';
    return '<section class="v09140-day-card"><div class="v09140-day-head"><div><span class="v09140-day-label">JORNADA</span><h3>'+dayLabel(day)+'</h3></div><strong>'+money(total)+'</strong></div><div class="v09140-day-metrics"><div><span>Oficial</span><strong>'+officialHours.toFixed(2)+' h</strong><small>'+money(officialCost)+'</small></div><div><span>Manobre</span><strong>'+laborerHours.toFixed(2)+' h</strong><small>'+money(laborerCost)+'</small></div><div><span>Registres</span><strong>'+hours.length+'</strong><small>línies d’hores</small></div><div class="is-total"><span>Total jornada</span><strong>'+money(total)+'</strong><small>guardat</small></div></div><div class="v09140-board-subtitle">Hores i personal</div><div class="table-wrap"><table class="v09140-board-table"><thead><tr><th>Concepte</th><th>Oficial h</th><th>€/h</th><th>Manobre h</th><th>€/h</th><th>Total</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div></section>';
  }
  function materialsDayCardHtml(day,materials,jobId){
    const total=materials.reduce((sum,item)=>sum+materialCost(item),0);
    const rows=materials.length?materials.map(item=>'<tr><td><strong>'+esc(item.concept||item.name||'Material')+'</strong><small>'+esc(item.source||item.notes||'')+'</small></td><td>'+esc(item.unit||'ud')+'</td><td class="num">'+num(item.qty||item.quantity).toFixed(3)+'</td><td class="num">'+money(item.unitPrice||item.price)+'</td><td class="num">'+money(materialCost(item))+'</td><td><button class="danger small" data-v09140-delete-material="'+esc(item.id)+'" data-v09140-job-id="'+esc(jobId)+'" data-v09140-board-tab="materials">Eliminar</button></td></tr>').join(''):'<tr><td colspan="6" class="muted">No hi ha materials registrats aquest dia.</td></tr>';
    return '<section class="v09140-day-card"><div class="v09140-day-head"><div><span class="v09140-day-label">MATERIALS</span><h3>'+dayLabel(day)+'</h3></div><strong>'+money(total)+'</strong></div><div class="v09140-day-metrics"><div><span>Línies</span><strong>'+materials.length+'</strong><small>materials</small></div><div><span>Quantitat</span><strong>'+materials.reduce((sum,item)=>sum+num(item.qty||item.quantity),0).toFixed(3)+'</strong><small>unitats totals</small></div><div><span>Preus</span><strong>'+materials.filter(item=>num(item.unitPrice||item.price)>0).length+'</strong><small>amb preu</small></div><div class="is-total"><span>Total materials</span><strong>'+money(total)+'</strong><small>guardat</small></div></div><div class="v09140-board-subtitle">Materials introduïts</div><div class="table-wrap"><table class="v09140-board-table"><thead><tr><th>Material</th><th>Ut.</th><th>Quant.</th><th>Preu/ut.</th><th>Total</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div></section>';
  }
  function timeBoardSummaryHtml(info){
    const rows=info.keys.filter(Boolean).map(day=>{
      const hours=info.hours.filter(item=>dateKeyFor(item)===day);
      const materials=info.materials.filter(item=>dateKeyFor(item)===day);
      const h=hours.reduce((sum,item)=>sum+hourCost(item),0);
      const m=materials.reduce((sum,item)=>sum+materialCost(item),0);
      return '<tr><td>'+esc(dayLabel(day))+'</td><td class="num">'+hours.reduce((sum,item)=>sum+num(item.officialHours)+num(item.laborerHours),0).toFixed(2)+' h</td><td class="num">'+money(h)+'</td><td class="num">'+materials.length+'</td><td class="num">'+money(m)+'</td><td class="num"><strong>'+money(h+m)+'</strong></td></tr>';
    }).join('');
    return '<div class="card v09140-summary-card"><div class="v09140-section-kicker">RESUM PER DIA</div><h3>Cost i rendiment registrat</h3>'+table(['Dia','Hores','Cost personal','Materials','Cost materials','Total jornada'],rows?[rows]:['<tr><td colspan="6" class="muted">Encara no hi ha jornades ni materials.</td></tr>'])+'</div>';
  }
  function boardEmpty(label,actionAttr,buttonLabel){
    return '<div class="card v09140-board-empty"><h3>'+label+'</h3><p class="muted">Encara no hi ha registres. Pots afegir-ne un des del botó superior.</p><button class="primary" type="button" '+actionAttr+'>'+buttonLabel+'</button></div>';
  }
  timeBoardHtml=function(jobId,tab='jornades'){
    const job=byId(data.jobs,jobId)||{};
    const client=byId(data.clients,job.clientId)||{};
    const info=timeBoardData(jobId);
    const active=['jornades','materials','resum'].includes(tab)?tab:'jornades';
    const totalHours=info.hours.reduce((sum,item)=>sum+num(item.officialHours)+num(item.laborerHours),0);
    const totalMaterials=info.materials.reduce((sum,item)=>sum+materialCost(item),0);
    const totalHoursCost=info.hours.reduce((sum,item)=>sum+hourCost(item),0);
    let body='';
    if(active==='jornades'){
      const keys=timeBoardDayKeys(info,'hours');
      body=keys.length?'<div class="v09140-days">'+keys.map(day=>hoursDayCardHtml(day,info.hours.filter(item=>dateKeyFor(item)===day),jobId)).join('')+'</div>':boardEmpty('Jornades i hores','data-v09140-add-hour="'+esc(jobId)+'"','+ Afegir jornada');
    }else if(active==='materials'){
      const keys=timeBoardDayKeys(info,'materials');
      body=keys.length?'<div class="v09140-days">'+keys.map(day=>materialsDayCardHtml(day,info.materials.filter(item=>dateKeyFor(item)===day),jobId)).join('')+'</div>':boardEmpty('Materials','data-v09140-add-material="'+esc(jobId)+'"','+ Afegir material');
    }else body=timeBoardSummaryHtml(info);
    const identity=practicalIdentity(job,null,client);
    return '<div class="v09140-timeboard" id="v09140TimeBoardRoot" data-job-id="'+esc(jobId)+'" data-v09140-board-tab="'+esc(active)+'"><div class="v09140-board-header"><div><span class="v09140-section-kicker">FITXA D’OBRA</span><h2>Temps i materials</h2><p>'+esc(identity.keyword)+'<br><span class="muted">'+esc([identity.address,identity.city].filter(Boolean).join(' · '))+'</span></p></div><div class="v09140-board-total"><span>Cost acumulat</span><strong>'+money(totalHoursCost+totalMaterials)+'</strong><small>guardat localment</small></div></div><div class="v09140-board-actions"><button class="ghost" type="button" data-v09140-close-timeboard>← Tornar a la fitxa</button><span class="v09140-saved-dot">● Canvis guardats automàticament</span></div><div class="grid three v09140-board-kpis"><div class="kpi"><span>Hores totals</span><strong>'+totalHours.toFixed(2)+' h</strong></div><div class="kpi"><span>Cost personal</span><strong>'+money(totalHoursCost)+'</strong></div><div class="kpi"><span>Materials</span><strong>'+money(totalMaterials)+'</strong></div></div><div class="v09140-board-tabs" role="tablist"><button class="'+(active==='jornades'?'active':'')+'" type="button" data-v09140-board-tab="jornades" data-v09140-job-id="'+esc(jobId)+'">Jornades</button><button class="'+(active==='materials'?'active':'')+'" type="button" data-v09140-board-tab="materials" data-v09140-job-id="'+esc(jobId)+'">Materials</button><button class="'+(active==='resum'?'active':'')+'" type="button" data-v09140-board-tab="resum" data-v09140-job-id="'+esc(jobId)+'">Resum per dia</button></div><div class="v09140-board-tab-actions"><div><strong>'+esc(active==='jornades'?'Hores i personal':active==='materials'?'Materials i albarans':'Visió general')+'</strong><span>Les altes s’obren en una pantalla nova i queden guardades en confirmar.</span></div><div class="actions"><button class="primary" type="button" data-v09140-add-hour="'+esc(jobId)+'">+ Afegir jornada</button><button class="ghost" type="button" data-v09140-add-material="'+esc(jobId)+'">+ Afegir material</button></div></div><div class="v09140-board-content">'+body+'</div></div>';
  };
  function closeTimeEntry(){ document.getElementById('v09140EntryOverlay')?.remove(); }
  function timeEntryHtml(jobId,kind){
    const job=byId(data.jobs,jobId)||{};
    const isHour=kind==='hour';
    const title=isHour?'Afegir jornada':'Afegir material';
    const kicker=isHour?'NOVA JORNADA':'NOU MATERIAL';
    const form=isHour?'<label>Dia<input name="date" type="date" value="'+todayKey()+'" required></label><label>Concepte<input name="concept" value="Jornada d’obra"></label><div class="grid two"><label>Oficial h<input name="officialHours" type="number" min="0" step="0.25" value="0" required></label><label>€/h oficial<input name="officialRate" type="number" min="0" step="0.01" value="'+esc(job.officialRate||data.settings.officialHourRate||31)+'" required></label><label>Manobre h<input name="laborerHours" type="number" min="0" step="0.25" value="0" required></label><label>€/h manobre<input name="laborerRate" type="number" min="0" step="0.01" value="'+esc(job.laborerRate||data.settings.laborerHourRate||27)+'" required></label></div><label>Notes<textarea name="notes" rows="4" placeholder="Incidències o observacions de la jornada"></textarea></label>':'<label>Dia<input name="date" type="date" value="'+todayKey()+'" required></label><label>Material / concepte<input name="concept" required placeholder="Rajola, morter, làmina..."></label><div class="grid two"><label>Unitat<input name="unit" value="ud"></label><label>Quantitat<input name="qty" type="number" min="0" step="0.001" value="1" required></label><label>Preu/ut. €<input name="unitPrice" type="number" min="0" step="0.01" value="0" required></label><label>Origen<input name="source" placeholder="Albarà / proveïdor"></label></div><label>Notes<textarea name="notes" rows="4" placeholder="Albarà, proveïdor o observacions"></textarea>';
    return '<div id="v09140EntryOverlay" class="v09140-entry-overlay" role="dialog" aria-modal="true"><div class="v09140-entry-modal"><div class="v09140-entry-header"><div><span class="v09140-section-kicker">'+kicker+'</span><h2>'+title+'</h2><p>'+esc(practicalIdentity(job,null,byId(data.clients,job.clientId)||{}).keyword)+'</p></div><button class="ghost" type="button" data-v09140-close-entry>× Tancar</button></div><form id="'+(isHour?'v09140HourBoardForm':'v09140MaterialBoardForm')+'" class="v09140-entry-form" data-job-id="'+esc(jobId)+'"><input type="hidden" name="jobId" value="'+esc(jobId)+'">'+form+'<div class="actions v09140-entry-actions"><button class="ghost" type="button" data-v09140-close-entry>Cancel·lar</button><button class="primary" type="submit">Guardar '+(isHour?'jornada':'material')+'</button></div></form></div></div>';
  }
  function openTimeEntry(jobId,kind){
    const job=byId(data.jobs,jobId);
    if(!job) return alert('No s’ha trobat aquesta obra.');
    closeTimeEntry();
    document.body.insertAdjacentHTML('beforeend',timeEntryHtml(jobId,kind));
    const overlay=document.getElementById('v09140EntryOverlay');
    overlay?.querySelectorAll('[data-v09140-close-entry]').forEach(button=>button.onclick=closeTimeEntry);
    if(overlay) overlay.onclick=event=>{ if(event.target===overlay) closeTimeEntry(); };
    const form=overlay?.querySelector('form');
    if(form) form.onsubmit=kind==='hour'?saveBoardHour:saveBoardMaterial;
    form?.querySelector('input:not([type="hidden"])')?.focus();
  }
  saveBoardHour=function(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const jobId=event.target.dataset.jobId||fields.jobId||'';
    if(!jobId) return alert('No s’ha pogut identificar l’obra.');
    const officialHours=num(fields.officialHours), laborerHours=num(fields.laborerHours);
    if(officialHours<=0 && laborerHours<=0) return alert('Indica almenys una hora d’oficial o de manobre.');
    const item={id:uid('HRS'),jobId,date:fields.date||todayKey(),officialHours,officialRate:num(fields.officialRate),laborerHours,laborerRate:num(fields.laborerRate),hours:officialHours+laborerHours,total:Number((officialHours*num(fields.officialRate)+laborerHours*num(fields.laborerRate)).toFixed(2)),concept:fields.concept||'Jornada d’obra',notes:fields.notes||'',status:'Manual'};
    data.hores.push(item);
    saveData();
    closeTimeEntry();
    openTimeBoard(jobId,'jornades');
  };
  saveBoardMaterial=function(event){
    event.preventDefault();
    const fields=formObj(event.target);
    const jobId=event.target.dataset.jobId||fields.jobId||'';
    if(!jobId) return alert('No s’ha pogut identificar l’obra.');
    const concept=tx(fields.concept);
    if(!concept) return alert('Indica el material o concepte.');
    const qty=num(fields.qty), unitPrice=num(fields.unitPrice);
    const item={id:uid('MAT'),jobId,date:fields.date||todayKey(),concept,name:concept,unit:fields.unit||'ud',qty,unitPrice,total:Number((qty*unitPrice).toFixed(2)),source:fields.source||'Manual',notes:fields.notes||'',status:'Manual'};
    data.materials.push(item);
    saveData();
    closeTimeEntry();
    openTimeBoard(jobId,'materials');
  };
  deleteBoardHour=function(id,jobId,tab='jornades'){
    if(!confirm('Eliminar aquesta imputació d’hores?')) return;
    data.hores=data.hores.filter(item=>item.id!==id);
    saveData();
    openTimeBoard(jobId,tab);
  };
  deleteBoardMaterial=function(id,jobId,tab='materials'){
    if(!confirm('Eliminar aquest material?')) return;
    data.materials=data.materials.filter(item=>item.id!==id);
    saveData();
    openTimeBoard(jobId,tab);
  };
  bindTimeBoard=function(){
    const root=document.getElementById('v09140TimeBoardRoot')||document;
    root.querySelectorAll('[data-v09140-close-timeboard]').forEach(button=>button.onclick=closeTimeBoard);
    root.querySelectorAll('[data-v09140-board-tab]').forEach(button=>button.onclick=()=>openTimeBoard(button.dataset.v09140JobId,button.dataset.v09140BoardTab));
    root.querySelectorAll('[data-v09140-add-hour]').forEach(button=>button.onclick=()=>openTimeEntry(button.dataset.v09140AddHour,'hour'));
    root.querySelectorAll('[data-v09140-add-material]').forEach(button=>button.onclick=()=>openTimeEntry(button.dataset.v09140AddMaterial,'material'));
    root.querySelectorAll('[data-v09140-delete-hour]').forEach(button=>button.onclick=()=>deleteBoardHour(button.dataset.v09140DeleteHour,button.dataset.v09140JobId,button.dataset.v09140BoardTab||'jornades'));
    root.querySelectorAll('[data-v09140-delete-material]').forEach(button=>button.onclick=()=>deleteBoardMaterial(button.dataset.v09140DeleteMaterial,button.dataset.v09140JobId,button.dataset.v09140BoardTab||'materials'));
  };
  openTimeBoard=function(jobId,tab='jornades'){
    ensureCollections();
    const job=byId(data.jobs,jobId);
    if(!job) return alert('No s’ha trobat aquesta obra.');
    state.v09140TimeBoardTab=tab;
    openModal(timeBoardHtml(jobId,tab));
    markTimeModal();
    bindTimeBoard();
  };
  const baseCloseModalV09140Final=closeModal;
  closeModal=function(){ closeTimeEntry(); return baseCloseModalV09140Final(); };
  teimor09136TimeMaterialsTab=function(model){
    const hours=arr(data.hores).filter(item=>item.jobId===model.job.id);
    const materials=arr(data.materials).filter(item=>item.jobId===model.job.id);
    const total=hours.reduce((sum,item)=>sum+hourCost(item),0)+materials.reduce((sum,item)=>sum+materialCost(item),0);
    return '<div class="v09140-time-launch"><div><span class="v09140-section-kicker">CONTROL DE COSTOS</span><h3>Temps i materials de l’obra</h3><p>Les jornades i els materials s’introdueixen en un tauler separat, amb pestanyes i una pantalla nova per a cada alta.</p></div><div class="v09140-launch-right"><strong>'+money(total)+'</strong><button class="primary" type="button" data-v09140-open-timeboard="'+esc(model.job.id)+'">Obrir tauler</button></div></div><div class="grid two"><div class="card"><h3>Jornades</h3><p class="muted">'+hours.length+' línies d’hores registrades.</p></div><div class="card"><h3>Materials</h3><p class="muted">'+materials.length+' materials registrats.</p></div></div>';
  };
  window.teimor09140OpenTimeBoard=openTimeBoard;
